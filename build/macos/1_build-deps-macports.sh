...
if [ -f "$OPT_PREFIX/bin/port" ]; then
  ...
  if echo "$CI_JOB_NAME" | grep -q 'part1'; then
    eval $( [ "$OPT_PREFIX" = /opt/local ] && echo sudo ) port install -vN $(grep -v '^#' build/macos/all-deps-uni.txt | sed 's/|homebrew:[^ ]*//g' | tr -d '\' | awk '{print} /gettext/{exit}' | xargs)
  elif echo "$CI_JOB_NAME" | grep -q 'part2'; then
    eval $( [ "$OPT_PREFIX" = /opt/local ] && echo sudo ) port install -vN $(grep -v '^#' build/macos/all-deps-uni.txt | sed 's/|homebrew:[^ ]*//g' | tr -d '\' | awk '{print} /gi-docgen/{exit}' | xargs)
  elif echo "$CI_JOB_NAME" | grep -q 'part3'; then
  ...
  elif echo "$CI_JOB_NAME" | grep -q 'part4'; then
