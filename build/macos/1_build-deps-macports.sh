...
if [ -f "$OPT_PREFIX/bin/port" ]; then
  ...
  brew_qoi="${OPT_PREFIX}/include/qoi.h"; if [ ! -f "$brew_qoi" ]; then curl -Lo "$brew_qoi" https://raw.githubusercontent.com/phoboslab/qoi/master/qoi.h; fi #see all-deps-uni.txt
...
self_build()
{
  ...
  if [ ! -d "$dep" ]; then
    if ( echo $1 | grep -q 'babl' || echo $1 | grep -q 'gegl' ) && [ "$CI_COMMIT_TAG" ]; then
      ...
      tag_branch=$(echo "$2" | grep -qv ".patch" && echo "${2:-master}" || echo "master")
    fi
...
self_build https://gitlab.gnome.org/tal.regev/gegl 'TalR/msys2_def' '-Db_lto=true -Dworkshop=true'
