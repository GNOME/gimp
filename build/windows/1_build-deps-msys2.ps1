...
if (-not $env:MSYSTEM_PREFIX -and $env:MSYS_ROOT)
  {
    $env:MSYSTEM_PREFIX = if ((Get-WmiObject Win32_ComputerSystem).SystemType -like 'ARM64*') { 'clangarm64' } else { 'ucrt64' }
  }
...
if (Test-Path "$env:VCPKG_ROOT\vcpkg.exe" -Type Leaf)
...
else
  {
    ...
    powershell -Command { $ProgressPreference = 'SilentlyContinue'; $env:PATH="$env:MSYS_ROOT\usr\bin;$env:PATH"; pacman --noconfirm -S --needed (Get-Content build/windows/all-deps-uni.txt | Select-String 'MINGW_' | ForEach-Object { ($_ -split '\|vcpkg:')[0] -replace '\$\{MINGW_PACKAGE_PREFIX\}', $(if ($env:MINGW_PACKAGE_PREFIX) { $env:MINGW_PACKAGE_PREFIX } elseif ($env:MSYSTEM_PREFIX -eq 'clangarm64') { 'mingw-w64-clang-aarch64' } else { 'mingw-w64-ucrt-x86_64' }) }) }; if ("$LASTEXITCODE" -gt '0') { exit 1 }
  }
            ...
            if (-not "$env:VCPKG_ROOT")
              {
                ...
                $clang_opts_meson=@('-Dc_args=-"gcodeview"', '-Dcpp_args=-"gcodeview"', '-Dc_link_args="-Wl,--pdb="', '-Dcpp_link_args="-Wl,--pdb="')
              }
