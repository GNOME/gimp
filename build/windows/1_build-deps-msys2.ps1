...
if (Test-Path "$env:VCPKG_ROOT\vcpkg.exe" -Type Leaf)
  {
    ...
    & "$env:VCPKG_ROOT\vcpkg.exe" remove aom; $env:VCPKG_DEFAULT_BINARY_CACHE="$env:VCPKG_ROOT/buildtrees/aom"; & "$env:VCPKG_ROOT\vcpkg.exe" install --no-print-usage --recurse aom; Remove-Item env:VCPKG_DEFAULT_BINARY_CACHE
