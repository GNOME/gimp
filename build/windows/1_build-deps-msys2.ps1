...
Copy-Item $MSYS2_PREFIX\etc\pacman.conf $MSYS2_PREFIX\etc\pacman.conf.bak
(Get-Content $MSYS2_PREFIX\etc\pacman.conf) | Foreach-Object {$_ -replace "SigLevel    = Required","SigLevel    = DatabaseNever"} |
Set-Content $MSYS2_PREFIX\etc\pacman.conf
...
Remove-Item $MSYS2_PREFIX\etc\pacman.conf
Rename-Item $MSYS2_PREFIX\etc\pacman.conf.bak $MSYS2_PREFIX\etc\pacman.conf
