...
if ($env:VCPKG_ROOT)
  {
    ...
    self_build https://gitlab.gnome.org/GNOME/gegl 'bruno/clean' @('build\windows\patches\0001-libs-operations-meson-Do-not-build-CTX-which-is-Unix.patch') @('-Dintrospection=false', "$WARN_AS_ERROR_ON_CI")
    exit 0
  }
self_build https://gitlab.gnome.org/GNOME/babl
self_build https://gitlab.gnome.org/GNOME/gegl 'bruno/clean' @('-Ddocs=false')
