...
if (-not $env:VCPKG_ROOT -or (Test-Path "$env:VCPKG_ROOT\vcpkg.exe" -Type Leaf))
  {
    $env:VCPKG_ROOT = $(Resolve-Path "${PWD}${PARENT_DIR}\vcpkg" -ErrorAction SilentlyContinue)
    if ((Test-Path "$env:VCPKG_ROOT\vcpkg.exe" -Type Leaf) -and -not "$env:VSINSTALLDIR")
      {
        Write-Host '(ERROR): MSVC installation not found. Please, install it then run this script from Developer PowerShell.' -ForegroundColor Red
        exit 1
      }
  }
...
Write-Output "$([char]27)[0Ksection_start:$(Get-Date -UFormat %s -Millisecond 0):deps_install[collapsed=true]$([char]13)$([char]27)[0KInstalling dependencies provided by $(if ("$env:VCPKG_ROOT" -ne '') {'vcpkg'} else {'MSYS2'})"
if (Test-Path "$env:VCPKG_ROOT\vcpkg.exe" -Type Leaf)
  {
    ...
    $env:PKG_CONFIG="$env:VCPKG_ROOT\installed\$env:VCPKG_DEFAULT_TRIPLET\tools\pkgconf\pkgconf.exe"
    $env:CC='clang-cl' 
  }
else
  {
    Write-Output '(INFO): vcpkg installation not found. Falling back to MSYS2...'
...
function self_build ([string]$repo, [array]$branch, [array]$patches, [array]$options)
  {
    ...
    if (-not (Test-Path _build-$(if ($env:VSINSTALLDIR) {$env:VCPKG_DEFAULT_TRIPLET} else {$env:MSYSTEM_PREFIX})\build.ninja -Type Leaf))
      {
        if ((Test-Path meson.build -Type Leaf) -and -not (Test-Path CMakeLists.txt -Type Leaf))
          {
            ...
            meson setup _build-$(if ($env:VSINSTALLDIR) {$env:VCPKG_DEFAULT_TRIPLET} else {$env:MSYSTEM_PREFIX}) -Dprefix="$GIMP_PREFIX" $PKGCONF_RELOCATABLE_OPTION `
        ...
        elseif (Test-Path CMakeLists.txt -Type Leaf)
          {
            ...
            cmake -G Ninja -B _build-$(if ($env:VSINSTALLDIR) {$env:VCPKG_DEFAULT_TRIPLET} else {$env:MSYSTEM_PREFIX}) -DCMAKE_INSTALL_PREFIX="$GIMP_PREFIX" `
                  -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_COLOR_DIAGNOSTICS=ON $clang_opts_cmake `
                  $(if ($branch -like '-*') { $branch } elseif ($patches -like '-*') { $patches } else { $options });
          }
        if ("$LASTEXITCODE" -gt '0') { exit 1 }
      }
    Set-Location _build-$(if ($env:VSINSTALLDIR) {$env:VCPKG_DEFAULT_TRIPLET} else {$env:MSYSTEM_PREFIX})
