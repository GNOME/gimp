...
DevelopmentWarning=Это разрабатываемая версия GIMP. Она не была протестирована так же, как стабильная версия, в результате GIMP может не работать должным образом. Просим Вас сообщить о возникших проблемах в GIMP bugzilla (компонент Installer):%n_https://bugzilla.gnome.org/enter_bug.cgi?product=GIMP%n%nПроблемы, известные в этой версии:%n- загрузка файлов TIFF не работает%n- размеры файлов отображаются неверно%nПожалуйста, не сообщайте об этих проблемах, мы уже знаем про них.%n%nВы хотите продолжить установку?
