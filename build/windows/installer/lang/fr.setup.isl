...
DevelopmentWarning=Ceci est une version de développement de l'installateur GIMP. Elle a moins été testée que l'installateur stable, ce qui peut causer des dysfonctionnements de GIMP. Veuillez rapporter les problèmes rencontrés dans le bugzilla GIMP (composant: "Installer"):%n_https://bugzilla.gnome.org/enter_bug.cgi?product=GIMP%n%nIl y a des problèmes connus dans cet installateur :%n- le chargement d'images TIFF ne fonctionne pas%n- les tailles de fichiers affichées sont incorrectes%nMerci de ne pas rapporter ces problèmes dont nous sommes déjà conscients.%n%nSouhaitez-vous tout de même poursuivre l'installation ?
