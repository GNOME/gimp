...
DevelopmentWarning=Esta es una versión de desarrollo del instalador de GIMP. No ha sido probado tan profundamente como el instalador estable, lo que puede resultar en que GIMP no funcione apropiadamente. Por favor reporte cualquier problema que usted encuentre en el bugzilla de GIMP (Installer component):%n_https://bugzilla.gnome.org/enter_bug.cgi?product=GIMP%n%nAlgunos problemas conocidos en este instalador son:%n- abrir archivos TIFF no funciona%n- el tamaño de los archivos no se muestra correctamente%nPor favor no reporte estos problemas, puesto que ya estamos al tanto de ellos.%n%n¿Desea continuar con la instalación de todos modos?
