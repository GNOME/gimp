...
DevelopmentWarning=Dies ist eine Entwicklerversion des GIMP-Installers. Er wurde nicht so intensiv wie der stabile Installer getestet, was dazu führen kann, dass GIMP nicht sauber arbeitet. Bitte melden Sie Probleme, auf die Sie stoßen im GIMP Bugzilla (Installationskomponente):%n_https://bugzilla.gnome.org/enter_bug.cgi?product=GIMP%n%nBekannte Probleme des Installers sind:%n- Laden von TIFF-Dateien funktioniert nicht%n- Dateigrößen werden nicht korrekt angezeigt%nBitte melden Sie diese Probleme nicht - wir kennen sie bereits.%n%nWollen Sie die Installation dennoch fortsetzen?
