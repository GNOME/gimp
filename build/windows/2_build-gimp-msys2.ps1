...
if (-not (Test-Path _build-$MSYSTEM_PREFIX\build.ninja -Type Leaf))
  {
    ...
    meson setup _build-$MSYSTEM_PREFIX -Dprefix="$GIMP_PREFIX" -Denable-default-bin=enabled -Dbuild-id='org.gimp.GIMP_official' `
                                       $INSTALLER_OPTION $STORE_OPTION $PKGCONF_RELOCATABLE_OPTION $NON_RELOCATABLE_OPTION
  }
