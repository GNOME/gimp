...
grep 'windows-installer=true' $MESON_BUILD_ROOT/meson-logs/meson-log.txt | export INSTALLER_OPTION_ON=1
grep 'ms-store=true' $MESON_BUILD_ROOT/meson-logs/meson-log.txt | export STORE_OPTION_ON=1
...
if [ "$PERFECT_BUNDLE" ]; then
  #NOTE: The bundling scripts need to set the ARTIFACTS_SUFFIX on CI: to our
  #dist scripts fallback code be able to identify what arch they are distributing
