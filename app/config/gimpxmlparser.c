...
/**
 * ligma_xml_parser_new:
 * @markup_parser: a #GMarkupParser
 * @user_data: user data to pass to #GMarkupParser functions
 *
 * LigmaXmlParser is a thin wrapper around GMarkupParser. This function
 * creates one for you and sets up a GMarkupParseContext.
 *
 * Returns: a new #LigmaXmlParser
 **/
LigmaXmlParser *
ligma_xml_parser_new (const GMarkupParser *markup_parser,
                     gpointer             user_data)
{
...
/**
 * ligma_xml_parser_parse_file:
 * @parser: a #LigmaXmlParser
 * @filename: name of a file to parse
 * @error: return location for possible errors
 *
 * This function creates a GIOChannel for @filename and calls
 * ligma_xml_parser_parse_io_channel() for you.
 *
 * Returns: %TRUE on success, %FALSE otherwise
 **/
gboolean
ligma_xml_parser_parse_file (LigmaXmlParser  *parser,
                            ...
                            GError        **error)
{
  ...
  success = ligma_xml_parser_parse_io_channel (parser, io, error);
 ...
 **/
gboolean
ligma_xml_parser_parse_gfile (LigmaXmlParser  *parser,
                             GFile          *file,
                             GError        **error)
{
  gchar    *path;
  ...
  success = ligma_xml_parser_parse_file (parser, path, error);
 ...
 **/
gboolean
ligma_xml_parser_parse_fd (LigmaXmlParser  *parser,
                          gint            fd,
                          GError        **error)
{
  GIOChannel *io;
...
/**
 * ligma_xml_parser_parse_io_channel:
 * @parser: a #LigmaXmlParser
 * @io: a #GIOChannel
 * @error: return location for possible errors
 *
 ...
 **/
gboolean
ligma_xml_parser_parse_io_channel (LigmaXmlParser  *parser,
                                  GIOChannel     *io,
                                  GError        **error)
{
  GIOStatus    status;
  ...
  if (g_strcmp0 (io_encoding, "UTF-8"))
    {
      g_warning ("ligma_xml_parser_parse_io_channel():\n"
 ...
 **/
gboolean
ligma_xml_parser_parse_buffer (LigmaXmlParser  *parser,
                              const gchar    *buffer,
                              gssize          len,
                              GError        **error)
{
 ...
 **/
void
ligma_xml_parser_free (LigmaXmlParser *parser)
{
