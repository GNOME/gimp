          ...
          if (ns_window)
            {
              ...
              [ns_window setStyleMask:minimize_mask];

              /* disable maximize button (while still preserving resizable attribute) */
              maximize_button = [ns_window standardWindowButton:NSWindowZoomButton];
              if (maximize_button)
...
#ifdef G_OS_WIN32
  hwnd_window = gdk_win32_window_get_handle (gtk_widget_get_window (GTK_WIDGET (dock_window)));
  if (hwnd_window)
    {
      HWND active_hwnd = GetActiveWindow ();
      if (hwnd_window && active_hwnd && hwnd_window != active_hwnd)
        {
          HWND current_parent = (HWND) GetWindowLongPtrW (hwnd_window, GWLP_HWNDPARENT);
          if (current_parent != active_hwnd)
            SetWindowLongPtrW (hwnd_window, GWLP_HWNDPARENT, (LONG_PTR) active_hwnd);
        }
    }
#endif
  ...
  if (display && gtk_widget_get_realized (GTK_WIDGET (dock_window)))
    {
      if (dock_window->p->context && dock_window->p->context->gimp)
        {
          Gimp          *gimp   = GIMP (dock_window->p->context->gimp);
