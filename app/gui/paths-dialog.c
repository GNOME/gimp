...
paths_dialog_create (void)
{
  ...
  g_signal_connect (paths_list, "event",
		    G_CALLBACK (paths_list_events),
		    paths_dialog);
  ...
  paths_dialog->selsigid =
    g_signal_connect (paths_list, "select_row",
		      G_CALLBACK (paths_select_row),
		      NULL);

  g_signal_connect (paths_list, "unselect_row",
  ...
  g_signal_connect (paths_dialog->vbox, "realize",
...
paths_dialog_realized (GtkWidget *widget)
{
  ...
  if (the_gimp->config->layer_preview_size < 64)
    dash_list[1] = 1;
		...
		gint  insrow)
{
  ...
  if (the_gimp->config->layer_preview_size)
    {
...
paths_dialog_preview_extents (void)
{
 ...
 if (! (gimage = paths_dialog->gimage))
  ...
  if (gimage->width > gimage->height)
    paths_dialog->ratio =
      (double) the_gimp->config->layer_preview_size / (double) gimage->width;
  else
    paths_dialog->ratio =
      (double) the_gimp->config->layer_preview_size / (double) gimage->height;
				   ...
				   gpointer   udata)
{
  ...
  g_signal_handlers_block_matched (paths_dialog->paths_list,
                                   G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL,
				   paths_dialog);
  gtk_clist_remove (GTK_CLIST (paths_dialog->paths_list), row);
  g_signal_handlers_unblock_matched (paths_dialog->paths_list,
                                     G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL,
				     paths_dialog);
  ...
  if (!plp->bz_paths || g_slist_length (plp->bz_paths) == 0)
    {
      g_signal_handler_disconnect (plp->gimage, plp->sig_id);
...
paths_update_preview (GimpBezierSelectTool *bezier_sel)
{
  ...
  if(paths_dialog &&
     ...
     the_gimp->config->layer_preview_size)
    {
      PathWidget *pwidget;
...
make_file_dlg (gpointer data) 
{
  ...
  g_signal_connect (GTK_FILE_SELECTION (file_dlg)->cancel_button, "clicked",
