...
session_init (Gimp *gimp)
{
  ...
  g_scanner_scope_add_symbol (scanner, 0,  "hide-tabs",
                              GINT_TO_POINTER (HIDE_TABS));
  ...
  while (g_scanner_peek_next_token (scanner) == token)
    {
      ...
      switch (token)
        ...
        case G_TOKEN_SYMBOL:
          ...
          else if (scanner->value.v_symbol == GINT_TO_POINTER (HIDE_TABS))
          {
            gboolean hide_tabs;

            token = G_TOKEN_IDENTIFIER;

            if (! gimp_scanner_parse_boolean (scanner, &hide_tabs))
              break;
              ...
              gboolean  always_save)
{
  ...
  gimp_config_writer_open (writer, "hide-tabs");
  gimp_config_writer_printf (writer,
                             GIMP_GUI_CONFIG (gimp->config)->hide_tabs ?
                             "yes" : "no");
  gimp_config_writer_close (writer);
