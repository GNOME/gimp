...
/*  event callback  */
static gint   ipal_area_events           (GtkWidget          *widget,
					  GdkEvent           *event,
					  GimpColormapDialog *ipal);

/*  create image menu  */
static void   image_menu_callback        (GtkWidget          *widget,
					  gpointer            data);
static GtkWidget * create_image_menu     (GimpColormapDialog *ipal,
					  GimpImage         **image,
					  gint               *,
					  GtkSignalFunc       );
static void   frame_size_alloc_cb        (GtkFrame           *frame,
					  GtkAllocation      *alloc,
					  GimpColormapDialog *ipal);
static void   window_size_req_cb         (GtkWindow          *win,
					  GtkRequisition     *req,
					  GimpColormapDialog *ipal);
...
static void   image_rename_cb            (GimpImage          *img,
					  GimpColormapDialog *ipal);
static void   image_cmap_change_cb       (GimpImage          *img,
					  gint                ncol,
					  GimpColormapDialog *ipal);
...
{
  GtkType gimp_colormap_dialog_type = 0;
  ...
  if (! gimp_colormap_dialog_type)
	...
	NULL,
      ...
      gimp_colormap_dialog_type = gtk_type_unique (GTK_TYPE_DIALOG, &info);
    }
...
GimpImage *
gimp_colormap_dialog_image (const GimpColormapDialog *colormap_dialog)
{
...
gint
gimp_colormap_dialog_col_index (const GimpColormapDialog *colormap_dialog)
{
