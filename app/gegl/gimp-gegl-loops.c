...
//2017/4/13 shark0r : According to my test, SSE decreases about 25% execution time
  ...
  *v_dest = (v_src1*_mm_set1_ps (src1_alpha) + v_src2*_mm_set1_ps (src2_alpha)) / _mm_set1_ps (result_alpha); 
      ...
      while (count--)
        {
          ...
          if (brush_a == 0) //pure smudge
            ...
            {
              gimp_gegl_smudge_with_paint_blend(brush_color_float, flow, accum, 1-flow, paint, no_erasing_effect);
            }
          accum  += 4;
          canvas += 4;
          paint  += 4;
