...
static void     gimp_select_by_content_tool_real_select         (GimpSelectByContentTool    *fst,
                                                           GimpDisplay           *display);
...
static void
gimp_select_by_content_tool_init (GimpSelectByContentTool *fst)
{
  GimpTool                  *tool = GIMP_TOOL (fst);
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
{
  GimpSelectByContentTool        *fst  = GIMP_SELECT_BY_CONTENT_TOOL (object);
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_get_segment (GimpSelectByContentTool  *fst,
                                   GimpVector2        **points,
                                   gint                *n_points,
                                   gint                 segment_start,
                                   gint                 segment_end)
...
static void
gimp_select_by_content_tool_get_segment_point (GimpSelectByContentTool *fst,
...
static void
gimp_select_by_content_tool_get_last_point (GimpSelectByContentTool *fst,
                                      gdouble            *start_point_x,
                                      gdouble            *start_point_y)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);

  gimp_select_by_content_tool_get_segment_point (fst,
                                           start_point_x,
                                           start_point_y,
...
static gboolean
gimp_select_by_content_tool_should_close (GimpSelectByContentTool *fst,
                                    GimpDisplay        *display,
                                    guint32             time,
                                    const GimpCoords   *coords)
{
...
static void
gimp_select_by_content_tool_handle_segment_selection (GimpSelectByContentTool *fst,
                                                GimpDisplay        *display,
                                                const GimpCoords   *coords)
{
  GimpSelectByContentToolPrivate *priv                  = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_revert_to_last_segment (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_remove_last_segment (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv      = GET_PRIVATE (fst);
  GimpDrawTool              *draw_tool = GIMP_DRAW_TOOL (fst);
...
static void
gimp_select_by_content_tool_add_point (GimpSelectByContentTool *fst,
                                 gdouble             x,
                                 gdouble             y)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_add_segment_index (GimpSelectByContentTool *fst,
                                         gint                index)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
static gboolean
gimp_select_by_content_tool_is_point_grabbed (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_fit_segment (GimpSelectByContentTool *fst,
...
static void
gimp_select_by_content_tool_move_segment_vertex_to (GimpSelectByContentTool *fst,
                                              gint                segment_index,
                                              gdouble             new_x,
                                              gdouble             new_y)
{
  ...
  if (segment_index > 0)
    {
      gimp_select_by_content_tool_get_segment (fst,
                                         &dest,
                                         &n_points,
      ...
      gimp_select_by_content_tool_fit_segment (fst,
                                         dest,
                                         *dest_start_target,
                                         *dest_end_target,
...
/**
 * gimp_select_by_content_tool_finish_line_segment:
 * @free_ploy_sel_tool:
 * @end_x:
 * @end_y:
 *
 ...
 **/
static void
gimp_select_by_content_tool_finish_line_segment (GimpSelectByContentTool *fst)
{
...
/**
 * gimp_select_by_content_tool_finish_free_segment:
 * @fst:
 *
 * Finnishes off the creation of a free segment.
 **/
static void
gimp_select_by_content_tool_finish_free_segment (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);

  /* The points are all setup, just make a segment */
  gimp_select_by_content_tool_add_segment_index (fst,
                                           priv->n_points - 1);
}

static void
gimp_select_by_content_tool_commit (GimpSelectByContentTool *fst,
                              GimpDisplay        *display)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
  ...
  if (priv->n_points >= 3)
    {
      gimp_select_by_content_tool_select (fst, display);
    }
...
static void
gimp_select_by_content_tool_revert_to_saved_state (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
  GimpVector2               *dest;
  gint                       n_points;
   ...
   * on, bail out
   */
  if (! gimp_select_by_content_tool_is_point_grabbed (fst))
    {
      return;
    }
...
static void
gimp_select_by_content_tool_handle_click (GimpSelectByContentTool *fst,
                                    const GimpCoords   *coords,
                                    guint32             time,
                                    GimpDisplay        *display)
{
  ...
  if (gimp_image_get_floating_selection (image))
  ...
  else
    {
      ...
      if (! gimp_select_by_content_tool_is_point_grabbed (fst))
        {
          gimp_select_by_content_tool_finish_line_segment (fst);
        }

      /* After the segments are up to date and we have handled
       * double-click, see if it's committing time
       */
      if (gimp_select_by_content_tool_should_close (fst,
                                              display,
                                              time,
                                              coords))
        {
           ...
           */
          gimp_select_by_content_tool_revert_to_saved_state (fst);

          gimp_select_by_content_tool_commit (fst, display);
        }
...
static void
gimp_select_by_content_tool_handle_normal_release (GimpSelectByContentTool *fst,
                                             const GimpCoords   *coords,
                                             GimpDisplay        *display)
{
  ...
  if (! gimp_select_by_content_tool_is_point_grabbed (fst))
    {
      gimp_select_by_content_tool_finish_free_segment (fst);
    }
...
static void
gimp_select_by_content_tool_handle_cancel (GimpSelectByContentTool *fst)
{
  if (gimp_select_by_content_tool_is_point_grabbed (fst))
    {
      gimp_select_by_content_tool_revert_to_saved_state (fst);
    ...
    {
      gimp_select_by_content_tool_remove_last_segment (fst);
    }
}

void
gimp_select_by_content_tool_select (GimpSelectByContentTool *fst,
                              GimpDisplay        *display)
{
  g_return_if_fail (GIMP_IS_SELECT_BY_CONTENT_TOOL (fst));
  g_return_if_fail (GIMP_IS_DISPLAY (display));

  GIMP_SELECT_BY_CONTENT_TOOL_GET_CLASS (fst)->select (fst,
                                                 display);
}

static void
gimp_select_by_content_tool_prepare_for_move (GimpSelectByContentTool *fst)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
  GimpVector2               *source;
  gint                       n_points;
...
static void
gimp_select_by_content_tool_update_motion (GimpSelectByContentTool *fst,
                                     gdouble             new_x,
                                     gdouble             new_y)
{
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
  ...
  if (gimp_select_by_content_tool_is_point_grabbed (fst))
    {
      ...
      gimp_select_by_content_tool_move_segment_vertex_to (fst,
                                                    priv->grabbed_segment_index,
                                                    new_x,
                                                    new_y);
  ...
  else
    {
      ...
      gimp_select_by_content_tool_add_point (fst,
                                       new_x,
                                       new_y);
    }
}
...
static void
gimp_select_by_content_tool_status_update (GimpSelectByContentTool *fst,
                                     GimpDisplay        *display,
                                     const GimpCoords   *coords,
                                     gboolean            proximity)
{
...
gimp_select_by_content_tool_control (GimpTool       *tool,
                               ...
                               GimpDisplay    *display)
{
  GimpSelectByContentTool        *fst  = GIMP_SELECT_BY_CONTENT_TOOL (tool);
  GimpSelectByContentToolPrivate *priv = GET_PRIVATE (fst);
                                   ...
                                   GimpDisplay      *display)
{
  ...
  gimp_select_by_content_tool_handle_segment_selection (fst,
                                                  display,
                                                  coords);
  hovering_first_point =
    gimp_select_by_content_tool_should_close (fst,
                                        display,
                                        NO_CLICK_TIME_AVAILABLE,
                                        coords);
  ...
  if (priv->n_points == 0 ||
      (gimp_select_by_content_tool_is_point_grabbed (fst) &&
       ! hovering_first_point) ||
      ! proximity)
    {
      priv->show_pending_point = FALSE;
          ...
          if (priv->constrain_angle && priv->n_points > 0)
            {
              ...
              gimp_select_by_content_tool_get_last_point (fst,
                                                    &start_point_x,
                                                    &start_point_y);
  ...
  if (tool->display == NULL)
    ...
    {
      gimp_select_by_content_tool_status_update (fst, display, coords, proximity);
    }
}
                                    ...
                                    GimpDisplay         *display)
{
  ...
  if (gimp_select_by_content_tool_is_point_grabbed (fst))
    {
      gimp_select_by_content_tool_prepare_for_move (fst);
    }
  else
    {
                                      ...
                                      GimpDisplay           *display)
{
  ...
  switch (release_type)
    ...
    case GIMP_BUTTON_RELEASE_NO_MOTION:
      ...
      gimp_select_by_content_tool_handle_click (fst,
                                          coords,
                                          time,
                                          display);
      break;
    ...
    case GIMP_BUTTON_RELEASE_NORMAL:
      gimp_select_by_content_tool_handle_normal_release (fst,
                                                   coords,
                                                   display);
      break;

    case GIMP_BUTTON_RELEASE_CANCEL:
      gimp_select_by_content_tool_handle_cancel (fst);
      break;
                              ...
                              GimpDisplay      *display)
{
  ...
  gimp_select_by_content_tool_update_motion (fst,
                                       coords->x,
                                       coords->y);
...
/**
 * gimp_select_by_content_tool_draw:
 * @draw_tool:
 *
...
{
  GimpSelectByContentTool        *fst                   = GIMP_SELECT_BY_CONTENT_TOOL (draw_tool);
  GimpSelectByContentToolPrivate *priv                  = GET_PRIVATE (fst);
...
static void
gimp_select_by_content_tool_real_select (GimpSelectByContentTool *fst,
                                   GimpDisplay        *display)
{
...
void
gimp_select_by_content_tool_get_points (GimpSelectByContentTool  *fst,
