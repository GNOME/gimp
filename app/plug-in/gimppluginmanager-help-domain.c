...
typedef struct _LigmaPlugInHelpDomain LigmaPlugInHelpDomain;

struct _LigmaPlugInHelpDomain
{
  GFile *file;
  gchar *domain_name;
  gchar *domain_uri;
...
void
ligma_plug_in_manager_help_domain_exit (LigmaPlugInManager *manager)
{
  GSList *list;
  ...
  for (list = manager->help_domains; list; list = list->next)
    {
      LigmaPlugInHelpDomain *domain = list->data;

      g_object_unref (domain->file);
      g_free (domain->domain_name);
      g_free (domain->domain_uri);
      g_slice_free (LigmaPlugInHelpDomain, domain);
    }
...
void
ligma_plug_in_manager_add_help_domain (LigmaPlugInManager *manager,
                                      GFile             *file,
                                      const gchar       *domain_name,
                                      const gchar       *domain_uri)
{
  LigmaPlugInHelpDomain *domain;

  g_return_if_fail (LIGMA_IS_PLUG_IN_MANAGER (manager));
  g_return_if_fail (G_IS_FILE (file));
  g_return_if_fail (domain_name != NULL);

  domain = g_slice_new (LigmaPlugInHelpDomain);
...
const gchar *
ligma_plug_in_manager_get_help_domain (LigmaPlugInManager  *manager,
                                      GFile              *file,
                                      const gchar       **domain_uri)
{
  GSList *list;
  ...
  /*  A NULL prog_name is LIGMA itself, return the default domain  */
  if (! file)
    return NULL;
...
gint
ligma_plug_in_manager_get_help_domains (LigmaPlugInManager   *manager,
                                       gchar             ***help_domains,
                                       gchar             ***help_uris)
{
  gint n_domains;
