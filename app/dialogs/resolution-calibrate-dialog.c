 ...
 * @resolution_entry: a #LigmaSizeEntry to connect the dialog to
 ...
 * is connected to a #LigmaSizeEntry handling the resolution to be set.
 **/
void
                             ...
                             const gchar *icon_name)
{
  ...
  g_return_if_fail (LIGMA_IS_SIZE_ENTRY (resolution_entry));
  ...
  dialog = ligma_dialog_new (_("Calibrate Monitor Resolution"),
                            "ligma-resolution-calibration",
  ...
  gdk_monitor_get_workarea (ligma_widget_get_monitor (dialog), &workarea);
  ...
  ligma_ruler_set_range (LIGMA_RULER (ruler), 0, ruler_width, ruler_width);
  ...
  ligma_ruler_set_range (LIGMA_RULER (ruler), 0, ruler_height, ruler_height);
  ...
  calibrate_xres =
    ligma_size_entry_get_refval (LIGMA_SIZE_ENTRY (resolution_entry), 0);
  calibrate_yres =
    ligma_size_entry_get_refval (LIGMA_SIZE_ENTRY (resolution_entry), 1);

  calibrate_entry =
    ligma_coordinates_new  (LIGMA_UNIT_INCH, "%p",
                           FALSE, FALSE, 10,
                           ...
                           1, LIGMA_MAX_IMAGE_SIZE,
                           0, 0);
  gtk_widget_hide (GTK_WIDGET (LIGMA_COORDINATES_CHAINBUTTON (calibrate_entry)));
    ...
    case GTK_RESPONSE_OK:
      {
        ...
        x = ligma_size_entry_get_refval (LIGMA_SIZE_ENTRY (calibrate_entry), 0);
        y = ligma_size_entry_get_refval (LIGMA_SIZE_ENTRY (calibrate_entry), 1);

        calibrate_xres = (gdouble) ruler_width  * calibrate_xres / x;
        calibrate_yres = (gdouble) ruler_height * calibrate_yres / y;

        chain_button = LIGMA_COORDINATES_CHAINBUTTON (resolution_entry);

        if (ABS (x - y) > LIGMA_MIN_RESOLUTION)
          ligma_chain_button_set_active (LIGMA_CHAIN_BUTTON (chain_button),
                                        FALSE);

        ligma_size_entry_set_refval (LIGMA_SIZE_ENTRY (resolution_entry),
                                    0, calibrate_xres);
        ligma_size_entry_set_refval (LIGMA_SIZE_ENTRY (resolution_entry),
                                    1, calibrate_yres);
      }
