...
#define ADD_TEST(function) \
  g_test_add_func ("/ligma-session-2-8-compatibility-multi-window/" #function, \
                   function);

#define SKIP_TEST(function) \
  g_test_add_func ("/ligma-session-2-8-compatibility-multi-window/subprocess/" #function, \
                   function);


/**
 * Tests that a single-window sessionrc in LIGMA 2.8 format is loaded
 * and written (thus also interpreted) like we expect.
 **/
static void
read_and_write_session_files (void)
{
  ligma_test_session_load_and_write_session_files ("sessionrc-2-8-multi-window",
                                                  "dockrc-2-8",
