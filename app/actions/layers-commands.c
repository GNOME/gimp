                               ...
                               gpointer    data)
{
  ...
  if (layers && layers->data)
    {
      parent   = GIMP_LAYER (gimp_item_get_parent (layers->data));
      position = gimp_item_get_index (layers->data);
    }
  ...
  for (iter = layers; iter; iter = iter->next)
    {
      if (! path)
      ...
      else
        {
          ...
          gimp_layer_vector_mask_set_invert (vector_mask,
                                             config->layer_add_mask_invert);
          ...
          /* To notify the Path Tool it should connect to the vector mask */
