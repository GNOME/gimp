...
static const LigmaVector2 Direction2Normal[4] =
{
    {  1.0f,  0.0f },
    { -1.0f,  0.0f },
    {  0.0f,  1.0f },
...
static void            ligma_line_art_finalize                  (GObject               *object);
static void            ligma_line_art_set_property              (GObject                *object,
                                                                guint                   property_id,
                                                                const GValue           *value,
                                                                GParamSpec             *pspec);
static void            ligma_line_art_get_property              (GObject                *object,
                                                                guint                   property_id,
                                                                GValue                 *value,
                                                                GParamSpec             *pspec);
...
static void            ligma_line_art_compute                   (LigmaLineArt            *line_art);
static void            ligma_line_art_compute_cb                (LigmaAsync              *async,
                                                                LigmaLineArt            *line_art);

static LigmaAsync     * ligma_line_art_prepare_async             (LigmaLineArt            *line_art,
                                                                gint                    priority);
static void            ligma_line_art_prepare_async_func        (LigmaAsync              *async,
                                                                LineArtData            *data);
...
static gboolean        ligma_line_art_idle                      (LigmaLineArt            *line_art);
static void            ligma_line_art_input_invalidate_preview  (LigmaViewable           *viewable,
                                                                LigmaLineArt            *line_art);


/* All actual computation functions. */

static GeglBuffer    * ligma_line_art_close                     (GeglBuffer             *buffer,
                                                                gboolean                select_transparent,
...
static void            ligma_lineart_denoise                    (GeglBuffer             *buffer,
                                                                int                     size,
                                                                LigmaAsync              *async);
static void            ligma_lineart_compute_normals_curvatures (GeglBuffer             *mask,
                                                                gfloat                 *normals,
                                                                gfloat                 *curvatures,
                                                                gfloat                 *smoothed_curvatures,
                                                                int                     normal_estimate_mask_size,
                                                                LigmaAsync              *async);
static gfloat        * ligma_lineart_get_smooth_curvatures      (GArray                 *edgelset,
                                                                LigmaAsync              *async);
static GArray        * ligma_lineart_curvature_extremums        (gfloat                 *curvatures,
                                                                gfloat                 *smoothed_curvatures,
                                                                gint                    curvatures_width,
                                                                gint                    curvatures_height,
                                                                LigmaAsync              *async);
static gint            ligma_spline_candidate_cmp               (const SplineCandidate  *a,
                                                                const SplineCandidate  *b,
                                                                gpointer                user_data);
static GList         * ligma_lineart_find_spline_candidates     (GArray                 *max_positions,
                                                                gfloat                 *normals,
                                                                gint                    width,
...
static GArray        * ligma_lineart_discrete_spline            (Pixel                   p0,
                                                                LigmaVector2             n0,
                                                                Pixel                   p1,
                                                                LigmaVector2             n1);

static gint            ligma_number_of_transitions               (GArray                 *pixels,
                                                                 GeglBuffer             *buffer);
static gboolean        ligma_line_art_allow_closure              (GeglBuffer             *mask,
                                                                 GArray                 *pixels,
                                                                 GList                 **fill_pixels,
                                                                 int                     significant_size,
                                                                 int                     minimum_size);
static GArray        * ligma_lineart_line_segment_until_hit      (const GeglBuffer       *buffer,
                                                                 Pixel                   start,
                                                                 LigmaVector2             direction,
                                                                 int                     size);
static gfloat        * ligma_lineart_estimate_strokes_radii      (GeglBuffer             *mask,
                                                                 LigmaAsync              *async);
static void            ligma_line_art_simple_fill                (GeglBuffer             *buffer,
                                                                 gint                    x,
                                                                 gint                    y,
                                                                 gint                   *counter);
...
static inline gboolean border_in_direction                      (GeglBuffer             *mask,
                                                                 Pixel                   p,
                                                                 int                     direction);
static inline LigmaVector2 pair2normal                           (Pixel                   p,
                                                                 gfloat                 *normals,
                                                                 gint                    width);

/* Edgel */

static Edgel    * ligma_edgel_new                  (int                x,
                                                   int                y,
                                                   Direction          direction);
static void       ligma_edgel_init                 (Edgel             *edgel);
static void       ligma_edgel_clear                (Edgel            **edgel);
static int        ligma_edgel_cmp                  (const Edgel       *e1,
                                                   const Edgel       *e2);
static guint      edgel2index_hash_fun            (Edgel              *key);
static gboolean   edgel2index_equal_fun           (Edgel              *e1,
                                                   Edgel              *e2);

static glong      ligma_edgel_track_mark           (GeglBuffer         *mask,
                                                   Edgel               edgel,
                                                   long                size_limit);
static glong      ligma_edgel_region_area          (const GeglBuffer   *mask,
                                                   Edgel               start_edgel);

/* Edgel set */

static GArray   * ligma_edgelset_new               (GeglBuffer         *buffer,
                                                   LigmaAsync          *async);
static void       ligma_edgelset_add               (GArray             *set,
                                                   int                 x,
                                                   int                 y,
                                                   Direction           direction,
                                                   GHashTable         *edgel2index);
static void       ligma_edgelset_init_normals      (GArray             *set);
static void       ligma_edgelset_smooth_normals    (GArray             *set,
                                                   int                 mask_size,
                                                   LigmaAsync          *async);
static void       ligma_edgelset_compute_curvature (GArray             *set,
                                                   LigmaAsync          *async);

static void       ligma_edgelset_build_graph       (GArray            *set,
                                                   GeglBuffer        *buffer,
                                                   GHashTable        *edgel2index,
                                                   LigmaAsync         *async);
static void       ligma_edgelset_next8             (const GeglBuffer  *buffer,
                                                   Edgel             *it,
                                                   Edgel             *n);

G_DEFINE_TYPE_WITH_CODE (LigmaLineArt, ligma_line_art, LIGMA_TYPE_OBJECT,
                         G_ADD_PRIVATE (LigmaLineArt))

#define parent_class ligma_line_art_parent_class

static guint ligma_line_art_signals[LAST_SIGNAL] = { 0 };

static void
ligma_line_art_class_init (LigmaLineArtClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  ligma_line_art_signals[COMPUTING_START] =
    g_signal_new ("computing-start",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_FIRST,
                  G_STRUCT_OFFSET (LigmaLineArtClass, computing_start),
                  NULL, NULL, NULL,
                  G_TYPE_NONE, 0);
  ligma_line_art_signals[COMPUTING_END] =
    g_signal_new ("computing-end",
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_FIRST,
                  G_STRUCT_OFFSET (LigmaLineArtClass, computing_end),
                  NULL, NULL, NULL,
                  G_TYPE_NONE, 0);

  object_class->finalize     = ligma_line_art_finalize;
  object_class->set_property = ligma_line_art_set_property;
  object_class->get_property = ligma_line_art_get_property;
  ...
  g_object_class_install_property (object_class, PROP_SELECT_TRANSPARENT,
                                   g_param_spec_boolean ("select-transparent",
                                                         ...
                                                         _("Select transparent pixels instead of gray ones"),
                                                         TRUE,
                                                         G_PARAM_CONSTRUCT | LIGMA_PARAM_READWRITE));
...
static void
ligma_line_art_init (LigmaLineArt *line_art)
{
  line_art->priv = ligma_line_art_get_instance_private (line_art);
}

static void
ligma_line_art_finalize (GObject *object)
{
  LigmaLineArt *line_art = LIGMA_LINE_ART (object);
...
static void
ligma_line_art_set_property (GObject      *object,
                            guint         property_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
      ...
      if (line_art->priv->select_transparent != g_value_get_boolean (value))
        {
          ...
          ligma_line_art_compute (line_art);
        }
      break;
    case PROP_MAX_GROW:
...
static void
ligma_line_art_get_property (GObject    *object,
                            guint       property_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
...
LigmaLineArt *
ligma_line_art_new (void)
{
  return g_object_new (LIGMA_TYPE_LINE_ART,
                       NULL);
}

void
ligma_line_art_bind_gap_length (LigmaLineArt *line_art,
                               gboolean     bound)
{
...
void
ligma_line_art_set_input (LigmaLineArt  *line_art,
                         LigmaPickable *pickable)
{
  g_return_if_fail (pickable == NULL || LIGMA_IS_VIEWABLE (pickable));
      ...
      if (pickable)
        {
          g_signal_connect (pickable, "invalidate-preview",
                            G_CALLBACK (ligma_line_art_input_invalidate_preview),
                            line_art);
        }
    }
}
...
LigmaPickable *
ligma_line_art_get_input (LigmaLineArt *line_art)
{
  return line_art->priv->input;
}

void
ligma_line_art_freeze (LigmaLineArt *line_art)
{
...
void
ligma_line_art_thaw (LigmaLineArt *line_art)
{
  g_return_if_fail (line_art->priv->frozen);
...
gboolean
ligma_line_art_is_frozen (LigmaLineArt *line_art)
{
  return line_art->priv->frozen;
}

GeglBuffer *
ligma_line_art_get (LigmaLineArt  *line_art,
                   gfloat      **distmap)
{
  ...
  if (line_art->priv->async)
    {
      ligma_waitable_wait (LIGMA_WAITABLE (line_art->priv->async));
    }
...
static void
ligma_line_art_compute (LigmaLineArt *line_art)
{
  ...
  if (line_art->priv->async)
    {
       ...
       * it might take a while to respond.  instead ligma_line_art_compute_cb()
       * bails if the async has been canceled, to avoid accessing the line art.
       */
      g_signal_emit (line_art, ligma_line_art_signals[COMPUTING_END], 0);
      ligma_cancelable_cancel (LIGMA_CANCELABLE (line_art->priv->async));
  ...
  if (line_art->priv->input)
    {
      /* ligma_line_art_prepare_async() will flush the pickable, which
      ...
      g_signal_handlers_block_by_func (
        ...
        G_CALLBACK (ligma_line_art_input_invalidate_preview),
        line_art);
      line_art->priv->async = ligma_line_art_prepare_async (line_art, +1);
      g_signal_emit (line_art, ligma_line_art_signals[COMPUTING_START], 0);
      g_signal_handlers_unblock_by_func (
        line_art->priv->input,
        G_CALLBACK (ligma_line_art_input_invalidate_preview),
        line_art);

      ligma_async_add_callback_for_object (line_art->priv->async,
                                          (LigmaAsyncCallback) ligma_line_art_compute_cb,
                                          line_art, line_art);
    }
}

static void
ligma_line_art_compute_cb (LigmaAsync   *async,
                          LigmaLineArt *line_art)
{
  ...
  if (ligma_async_is_finished (async))
    {
      ...
      result = ligma_async_get_result (async);
...
static LigmaAsync *
ligma_line_art_prepare_async (LigmaLineArt *line_art,
                             gint         priority)
{
  GeglBuffer  *buffer;
  LigmaAsync   *async;
  ...
  g_return_val_if_fail (LIGMA_IS_PICKABLE (line_art->priv->input), NULL);

  ligma_pickable_flush (line_art->priv->input);

  buffer = ligma_gegl_buffer_dup (
    ligma_pickable_get_buffer (line_art->priv->input));

  data  = line_art_data_new (buffer, line_art);

  g_object_unref (buffer);

  async = ligma_parallel_run_async_full (
    priority,
    (LigmaRunAsyncFunc) ligma_line_art_prepare_async_func,
...
static void
ligma_line_art_prepare_async_func (LigmaAsync   *async,
                                  LineArtData *data)
{
  GeglBuffer *buffer;
  ...
  closed = ligma_line_art_close (buffer,
                                select_transparent,
                                data->threshold,
  ...
  LIGMA_TIMER_END("close line-art");

  if (buffer != data->buffer)
    g_object_unref (buffer);

  if (! ligma_async_is_stopped (async))
    {
      if (buffer_x != 0 || buffer_y != 0)
        {
          buffer = g_object_new (GEGL_TYPE_BUFFER,
...
static gboolean
ligma_line_art_idle (LigmaLineArt *line_art)
{
  line_art->priv->idle_id = 0;
...
static void
ligma_line_art_input_invalidate_preview (LigmaViewable *viewable,
                                        LigmaLineArt  *line_art)
{
  if (! line_art->priv->idle_id)
    {
      line_art->priv->idle_id = g_idle_add_full (
        ...
        (GSourceFunc) ligma_line_art_idle,
        line_art, NULL);
    }
}
 ...
 * @async: the #LigmaAsync associated with the computation
 *
 ...
 */
static GeglBuffer *
ligma_line_art_close (GeglBuffer  *buffer,
                     gboolean     select_transparent,
                     gdouble      stroke_threshold,
                     gboolean     automatic_closure,
                     gint         spline_max_length,
                     ...
                     LigmaAsync   *async)
{
  ...
  strokes = gegl_buffer_new (gegl_buffer_get_extent (buffer),
                             gray_format);
  ligma_gegl_buffer_copy (buffer, NULL, GEGL_ABYSS_NONE, strokes, NULL);
  ...
  ligma_lineart_denoise (strokes, minimal_lineart_area, async);
  if (ligma_async_is_stopped (async))
    goto end1;
  ...
  if (automatic_closure &&
      (spline_max_length > 0 || segment_max_length > 0))
    {
      ...
      ligma_lineart_compute_normals_curvatures (strokes, normals, curvatures,
                                               smoothed_curvatures,
                                               normal_estimate_mask_size,
                                               async);
      if (ligma_async_is_stopped (async))
      ...
      radii = ligma_lineart_estimate_strokes_radii (strokes, async);
      if (ligma_async_is_stopped (async))
        goto end2;
      ...
      keypoints = ligma_lineart_curvature_extremums (curvatures, smoothed_curvatures,
                                                    width, height, async);
      if (ligma_async_is_stopped (async))
        goto end2;
      ...
      if (spline_max_length > 0)
        {
          ...
          candidates = ligma_lineart_find_spline_candidates (keypoints, normals, width,
                                                            spline_max_length,
                                                            spline_max_angle,
                                                            async);
          if (ligma_async_is_stopped (async))
          ...
          g_object_unref (closed);
          closed = ligma_gegl_buffer_dup (strokes);
          ...
          while (candidates)
            {
              ...
              if ((! g_hash_table_contains (visited, p1) ||
                   ...
                   GPOINTER_TO_INT (g_hash_table_lookup (visited, p2)) < end_point_connectivity))
                {
                  ...
                  LigmaVector2  vect1 = pair2normal (*p1, normals, width);
                  LigmaVector2  vect2 = pair2normal (*p2, normals, width);
                  gfloat       distance = ligma_vector2_length_val (ligma_vector2_sub_val (*p1, *p2));
                  gint         transitions;

                  ligma_vector2_mul (&vect1, distance);
                  ligma_vector2_mul (&vect1, spline_roundness);
                  ligma_vector2_mul (&vect2, distance);
                  ligma_vector2_mul (&vect2, spline_roundness);

                  discrete_curve = ligma_lineart_discrete_spline (*p1, vect1, *p2, vect2);

                  transitions = allow_self_intersections ?
                    ligma_number_of_transitions (discrete_curve, strokes) :
                    ligma_number_of_transitions (discrete_curve, closed);

                  if (transitions == 2 &&
                      ligma_line_art_allow_closure (closed, discrete_curve,
                                                   &fill_pixels,
                                                   created_regions_significant_area,
                                                   created_regions_minimum_area))
                    {
          ...
          for (i = 0; i < keypoints->len; i++)
            {
              ...
              *p = *point;
              ...
              if (! g_hash_table_contains (visited, p) ||
                ...
                {
                  GArray *segment = ligma_lineart_line_segment_until_hit (closed, *point,
                                                                         pair2normal (*point, normals, width),
                                                                         segment_max_length);

                  if (segment->len &&
                      ligma_line_art_allow_closure (closed, segment, &fill_pixels,
                                                   created_regions_significant_area,
                                                   created_regions_minimum_area))
                    {
                      gint j;
      ...
      for (iter = fill_pixels; iter; iter = iter->next)
        {
           ...
           * ligma_drawable_bucket_fill() to work on any buffer (the code
           * is already mostly there) rather than reimplementing a naive
           * bucket fill.
           * This is mostly a quick'n dirty first implementation which I
           * will improve later.
           */
          ligma_line_art_simple_fill (closed, (gint) p->x, (gint) p->y, &fill_max);
        }
...
static void
ligma_lineart_denoise (GeglBuffer *buffer,
                      int         minimum_area,
                      LigmaAsync  *async)
{
...
static void
ligma_lineart_compute_normals_curvatures (GeglBuffer *mask,
                                         gfloat     *normals,
                                         gfloat     *curvatures,
                                         gfloat     *smoothed_curvatures,
                                         int         normal_estimate_mask_size,
                                         LigmaAsync  *async)
{
  ...
  es = ligma_edgelset_new (mask, async);
  if (ligma_async_is_stopped (async))
    goto end;

  e = (Edgel **) es->data;

  ligma_edgelset_smooth_normals (es, normal_estimate_mask_size, async);
  if (ligma_async_is_stopped (async))
    goto end;

  ligma_edgelset_compute_curvature (es, async);
  if (ligma_async_is_stopped (async))
    goto end;
  ...
  edgels_curvatures = ligma_lineart_get_smooth_curvatures (es, async);
  if (ligma_async_is_stopped (async))
    goto end;
...
static gfloat *
ligma_lineart_get_smooth_curvatures (GArray    *edgelset,
                                    LigmaAsync *async)
{
  Edgel  **e;
 ...
 */
static GArray *
ligma_lineart_curvature_extremums (gfloat    *curvatures,
                                  gfloat    *smoothed_curvatures,
                                  gint       width,
                                  gint       height,
                                  LigmaAsync *async)
          ...
          if ((curvatures[x + y * width] > 0.0) && ! visited[x + y * width])
            {
              ...
              max_smoothed_curvature_pixel = ligma_vector2_new (-1.0, -1.0);
              max_smoothed_curvature       = 0.0f;

              max_raw_curvature_pixel = ligma_vector2_new (x, y);
...
static gint
ligma_spline_candidate_cmp (const SplineCandidate *a,
                           const SplineCandidate *b,
                           gpointer               user_data)
{
...
static GList *
ligma_lineart_find_spline_candidates (GArray    *max_positions,
                                     gfloat    *normals,
                                     gint       width,
                                     gint       distance_threshold,
                                     gfloat     max_angle_deg,
      ...
      for (j = i + 1; j < max_positions->len; j++)
        {
          ...
          const float distance = ligma_vector2_length_val (ligma_vector2_sub_val (p1, p2));

          if (distance <= distance_threshold)
            {
              LigmaVector2 normalP1;
              LigmaVector2 normalP2;
              LigmaVector2 p1f;
              LigmaVector2 p2f;
              LigmaVector2 p1p2;
              float       cosN;
              ...
              normalP1 = ligma_vector2_new (normals[((gint) p1.x + (gint) p1.y * width) * 2],
                                           normals[((gint) p1.x + (gint) p1.y * width) * 2 + 1]);
              normalP2 = ligma_vector2_new (normals[((gint) p2.x + (gint) p2.y * width) * 2],
                                           normals[((gint) p2.x + (gint) p2.y * width) * 2 + 1]);
              p1f = ligma_vector2_new (p1.x, p1.y);
              p2f = ligma_vector2_new (p2.x, p2.y);
              p1p2 = ligma_vector2_sub_val (p2f, p1f);

              cosN = ligma_vector2_inner_product_val (normalP1, (ligma_vector2_neg_val (normalP2)));
              qualityA = MAX (0.0f, 1 - distance / distance_threshold);
              qualityB = MAX (0.0f,
                              (float) (ligma_vector2_inner_product_val (normalP1, p1p2) - ligma_vector2_inner_product_val (normalP2, p1p2)) /
                              distance);
              ...
              if (quality > 0)
                {
                  ...
                  candidates = g_list_insert_sorted_with_data (candidates, candidate,
                                                               (GCompareDataFunc) ligma_spline_candidate_cmp,
                                                               NULL);
                }
            }
        }
...
static GArray *
ligma_lineart_discrete_spline (Pixel       p0,
                              LigmaVector2 n0,
                              Pixel       p1,
                              LigmaVector2 n1)
{
  ...
  Pixel         point = ligma_vector2_new ((gint) round (d0), (gint) round (d1));
  ...
  while (t <= 1.0)
    {
      ...
      double       dx;
      double       dy;
      Pixel        p = ligma_vector2_new ((gint) round (a0 * t3 + b0 * t2 + c0 * t + d0),
                                         (gint) round (a1 * t3 + b1 * t2 + c1 * t + d1));

      /* create ligma_vector2_neq () ? */
...
static gint
ligma_number_of_transitions (GArray     *pixels,
                            GeglBuffer *buffer)
{
  int result = 0;
 ...
 */
static gboolean
ligma_line_art_allow_closure (GeglBuffer *mask,
                             GArray     *pixels,
                             GList     **fill_pixels,
                             int         significant_size,
                             int         minimum_size)
      ...
      for (int direction = 0; direction < 4; ++direction)
        {
          if (p.x >= 0 && p.x < gegl_buffer_get_width (mask) &&
              ...
              border_in_direction (mask, p, direction))
            {
              ...
              count = ligma_edgel_track_mark (mask, e, max_edgel_count);
              if ((count != -1) && (count <= max_edgel_count))
                {
                  area = ligma_edgel_region_area (mask, e);
...
static GArray *
ligma_lineart_line_segment_until_hit (const GeglBuffer *mask,
                                     Pixel             start,
                                     LigmaVector2       direction,
                                     int               size)
{
  ...
  GArray      *points = g_array_new (FALSE, TRUE, sizeof (Pixel));
  int          tmax;
  LigmaVector2  p0 = ligma_vector2_new (start.x, start.y);

  ligma_vector2_mul (&direction, (gdouble) size);
  ...
  for (int t = 0; t <= tmax; ++t)
    {
      LigmaVector2 v = ligma_vector2_add_val (p0, ligma_vector2_mul_val (direction, (float)t / tmax));
      Pixel       p;
...
static gfloat *
ligma_lineart_estimate_strokes_radii (GeglBuffer *mask,
                                     LigmaAsync  *async)
{
  GeglBufferIterator *gi;
  gfloat             *dist;
...
static void
ligma_line_art_simple_fill (GeglBuffer *buffer,
                           gint        x,
                           gint        y,
                           gint       *counter)
{
  ...
  if (! val)
    {
      ...
      gegl_buffer_set (buffer, GEGL_RECTANGLE (x, y, 1, 1), 0,
                       NULL, &val, GEGL_AUTO_ROWSTRIDE);
      (*counter)--;
      ligma_line_art_simple_fill (buffer, x + 1, y, counter);
      ligma_line_art_simple_fill (buffer, x - 1, y, counter);
      ligma_line_art_simple_fill (buffer, x, y + 1, counter);
      ligma_line_art_simple_fill (buffer, x, y - 1, counter);
    }
}
...
pair2normal (Pixel   p,
             ...
             gint    width)
{
  return ligma_vector2_new (normals[((gint) p.x + (gint) p.y * width) * 2],
                           normals[((gint) p.x + (gint) p.y * width) * 2 + 1]);
}
/* Edgel functions */

static Edgel *
ligma_edgel_new (int       x,
                int       y,
                Direction direction)
{
  ...
  ligma_edgel_init (edgel);

  return edgel;
}

static void
ligma_edgel_init (Edgel *edgel)
{
  edgel->x_normal  = 0;
  edgel->y_normal  = 0;
  edgel->curvature = 0;
...
static void
ligma_edgel_clear (Edgel **edgel)
{
  g_clear_pointer (edgel, g_free);
}

static int
ligma_edgel_cmp (const Edgel* e1,
                const Edgel* e2)
{
  ligma_assert (e1 && e2);
 ...
 */
static glong
ligma_edgel_track_mark (GeglBuffer *mask,
                       Edgel       edgel,
                       long        size_limit)
{
  Edgel start = edgel;
  ...
  do
    {
      ...
      ligma_edgelset_next8 (mask, &edgel, &edgel);
      ...
      if (ligma_edgel_cmp (&edgel, &start) != 0)
        ++count;
    }
  while (ligma_edgel_cmp (&edgel, &start) != 0 && count <= size_limit);

  return count;
}

/**
 * ligma_edgel_region_area:
 ...
 */
static glong
ligma_edgel_region_area (const GeglBuffer *mask,
                        Edgel             start_edgel)
{
  ...
  while (ligma_edgel_cmp (&edgel, &start_edgel) != 0);
...
static GArray *
ligma_edgelset_new (GeglBuffer *buffer,
                   LigmaAsync  *async)
{
  GeglBufferIterator *gi;
  GArray             *set;
  ...
  g_array_set_clear_func (set, (GDestroyNotify) ligma_edgel_clear);
        ...
        for (x = startx; x < endx; x++)
          {
            if (*(p++))
              {
                if (! *prevy)
                  ligma_edgelset_add (set, x, y, YMinusDirection, edgel2index);
                if (! *nexty)
                  ligma_edgelset_add (set, x, y, YPlusDirection, edgel2index);
                if (! *prevx)
                  ligma_edgelset_add (set, x, y, XMinusDirection, edgel2index);
                if (! *nextx)
                  ligma_edgelset_add (set, x, y, XPlusDirection, edgel2index);
              }
            prevy++;
            nexty++;
            prevx++;
  ...
  ligma_edgelset_build_graph (set, buffer, edgel2index, async);
  if (ligma_async_is_stopped (async))
    goto end;

  ligma_edgelset_init_normals (set);
...
static void
ligma_edgelset_add (GArray     *set,
                   int         x,
                   int         y,
                   Direction   direction,
                   GHashTable *edgel2index)
{
  Edgel         *edgel    = ligma_edgel_new (x, y, direction);
...
static void
ligma_edgelset_init_normals (GArray *set)
{
  Edgel **e = (Edgel**) set->data;

  while (*e)
    {
      LigmaVector2 n = Direction2Normal[(*e)->direction];
...
static void
ligma_edgelset_smooth_normals (GArray    *set,
                              int        mask_size,
                              LigmaAsync *async)
{
  ...
  gfloat       weights[65];
  LigmaVector2  smoothed_normal;
  gint         i;

  ligma_assert (mask_size <= 65);
  ...
  for (i = 0; i < set->len; i++)
    {
      ...
      while (n-- && (edgel_after != edgel_before))
        {
          smoothed_normal = ligma_vector2_add_val (smoothed_normal,
                                                  ligma_vector2_mul_val (Direction2Normal[edgel_before->direction], weights[i]));
          smoothed_normal = ligma_vector2_add_val (smoothed_normal,
                                                  ligma_vector2_mul_val (Direction2Normal[edgel_after->direction], weights[i]));
          edgel_before = g_array_index (set, Edgel *, edgel_before->previous);
          edgel_after  = g_array_index (set, Edgel *, edgel_after->next);
          ++i;
        }
      ligma_vector2_normalize (&smoothed_normal);
...
static void
ligma_edgelset_compute_curvature (GArray    *set,
                                 LigmaAsync *async)
{
  gint i;
  ...
  for (i = 0; i < set->len; i++)
    {
      ...
      LigmaVector2  n_prev   = ligma_vector2_new (previous->x_normal, previous->y_normal);
      LigmaVector2  n_next   = ligma_vector2_new (next->x_normal, next->y_normal);
      LigmaVector2  diff     = ligma_vector2_mul_val (ligma_vector2_sub_val (n_next, n_prev),
                                                    0.5);
      const float  c        = ligma_vector2_length_val (diff);
...
static void
ligma_edgelset_build_graph (GArray     *set,
                           GeglBuffer *buffer,
                           GHashTable *edgel2index,
                           LigmaAsync  *async)
{
  ...
  for (i = 0; i < set->len; i++)
    {
      ...
      ligma_edgelset_next8 (buffer, it, &edgel);

      ligma_assert (g_hash_table_contains (edgel2index, &edgel));
...
static void
ligma_edgelset_next8 (const GeglBuffer *buffer,
                     Edgel            *it,
                     Edgel            *n)
{
  guint8 pixels[9];
