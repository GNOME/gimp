...
gimp_layer_vector_mask_render_path (GimpLayerVectorMask *layer_mask)
{
  ...
  if (layer_mask->invert)
    {
      gimp_drawable_fill (GIMP_DRAWABLE (layer_mask),
                          gimp_get_user_context (image->gimp),
                          GIMP_FILL_WHITE);

      fill_color = gegl_color_new ("black");
    }
  else
    {
...
void
gimp_layer_vector_mask_set_invert (GimpLayerVectorMask *layer_mask,
                                   gboolean             invert)
{
  g_return_if_fail (GIMP_IS_LAYER_VECTOR_MASK (layer_mask));

  layer_mask->invert = invert;
}
