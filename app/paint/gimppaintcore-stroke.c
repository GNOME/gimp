...
#include "ligmapaintcore-stroke.h"
#include "ligmapaintoptions.h"

#include "ligma-intl.h"


static void ligma_paint_core_stroke_emulate_dynamics (LigmaCoords *coords,
                                                     gint        length);
...
gboolean
ligma_paint_core_stroke (LigmaPaintCore     *core,
                        ...
                        GError           **error)
{
  ...
  if (ligma_paint_core_start (core, drawables, paint_options, &strokes[0],
                             error))
    {
      gint i;
      ...
      ligma_paint_core_paint (core, drawables, paint_options,
                             LIGMA_PAINT_STATE_FINISH, 0);

      ligma_paint_core_finish (core, drawables, push_undo);
...
gboolean
ligma_paint_core_stroke_boundary (LigmaPaintCore      *core,
                                 LigmaDrawable       *drawable,
                                 LigmaPaintOptions   *paint_options,
                                 ...
                                 GError            **error)
{
  ...
  coords = g_new0 (LigmaCoords, n_bound_segs + 4);
  ...
  for (s = 0; s < n_stroke_segs; s++)
    {
      ...
      if (emulate_dynamics)
        ligma_paint_core_stroke_emulate_dynamics (coords, n_coords);

      if (initialized ||
          ligma_paint_core_start (core, drawables, paint_options, &coords[0],
                                 error))
        {
          gint i;
...
gboolean
ligma_paint_core_stroke_vectors (LigmaPaintCore     *core,
                                LigmaDrawable      *drawable,
                                LigmaPaintOptions  *paint_options,
                                gboolean           emulate_dynamics,
                                ...
                                GError           **error)
{
  ...
  ligma_item_get_offset (LIGMA_ITEM (vectors),  &vectors_off_x, &vectors_off_y);
  ligma_item_get_offset (LIGMA_ITEM (drawable), &off_x, &off_y);
  ...
  for (stroke = vectors->strokes->head;
       ...
       stroke = stroke->next)
    {
      ...
      coords = ligma_stroke_interpolate (LIGMA_STROKE (stroke->data),
                                        1.0, &closed);
      ...
      if (coords && coords->len)
        {
          ...
          for (i = 0; i < coords->len; i++)
            {
              g_array_index (coords, LigmaCoords, i).x -= off_x;
              g_array_index (coords, LigmaCoords, i).y -= off_y;
            }

          if (emulate_dynamics)
            ligma_paint_core_stroke_emulate_dynamics ((LigmaCoords *) coords->data,
                                                     coords->len);

          if (initialized ||
              ligma_paint_core_start (core, drawables, paint_options,
                                     &g_array_index (coords, LigmaCoords, 0),
                                     error))
            {
              initialized = TRUE;

              core->cur_coords  = g_array_index (coords, LigmaCoords, 0);
              core->last_coords = g_array_index (coords, LigmaCoords, 0);
              ...
              for (i = 1; i < coords->len; i++)
                {
                  ligma_paint_core_interpolate (core, drawables, paint_options,
                                               &g_array_index (coords, LigmaCoords, i),
                                               0);
                }
...
static void
ligma_paint_core_stroke_emulate_dynamics (LigmaCoords *coords,
                                         gint        length)
{
  ...
  if (length > 1)
    {
      ...
      for (i = 1; i < length; i++)
        {
          coords[i].direction = ligma_coords_direction (&coords[i-1], &coords[i]);
        }
