                             ...
                             const gdouble     precision)
{
  ...
  for (i = 0; i < points->len - 1; i++ )
    {
       gimp_bezier_coords_difference (&(g_array_index (points, GimpCoords, i)),
                                      &(g_array_index (points, GimpCoords, i+1)),
                                      &difference);
       length += gimp_bezier_coords_length (&difference);
    }
  ...
  for (i=0; i < points->len - 1; i++)
    {
      gimp_bezier_coords_difference (&(g_array_index (points, GimpCoords , i)),
                                     &(g_array_index (points, GimpCoords , i+1)),
                                     &difference);
      segment_length = gimp_bezier_coords_length (&difference);

      if (length + segment_length < dist )
        {
      ...
      else
        {
          ...
          gimp_bezier_coords_mix ( 1 - u,
                                   &(g_array_index (points, GimpCoords , i)),
                                   u,
                                   &(g_array_index (points, GimpCoords , i+1)),
                                   position);
