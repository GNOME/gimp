/**********************************************************************
 *  Mandelbrot Chaos Fractal Explorer Plug-in (Version 1.00)
...
static void      mandelbrot 	    (GDrawable  *drawable);
static void      mandelbrot_render_row  (const guchar *src_row,
        			     guchar *dest_row,
        			     gint row,
        			     gint row_width,
        			     gint bytes);
static void      mandelbrot_get_pixel(int x, int y, guchar *pixel);
...
static void      mandelbrot_toggle_update    (GtkWidget *widget,
        				    gpointer   data);
static float xmin=-2;
static float xmax=1;

static float ymin=-1.5;
static float ymax=1.5;
static float xbild;
static float ybild;
static float xdiff;
static float ydiff;
...
static mandelbrot_interface_t wint = {
        NULL,  /* preview */
        NULL,  /* image */
        NULL,  /* wimage */
        FALSE  /* run */
...
static mandelbrot_vals_t wvals = {
        -2.0,1.0,-1.5,1.5,50.0,0,128,128,128,1,1,0,
}; /* wvals */
...
GtkWidget * mandelbrot_logo_dialog(void);
...
query ()
{
  ...
  gimp_install_procedure ("plug_in_mandelbrot",
        		  ...
        		  "Daniel Cotting (cotting@mygale.org, www.mygale.org/~cotting)",
        		  "1th May 1997",
        		  "<Image>/Filters/Render/Mandelbrot Fractal",
        		  "RGB*",
        		  PROC_PLUG_IN,
        		  nargs, nreturn_vals,
        		  args, return_vals);
  ...
  yhsiz = (double) (sel_height - 1) / 2.0;
        ...
        switch (run_mode) {
        	case RUN_INTERACTIVE:
        		...
        		gimp_get_data("plug_in_mandelbrot", &wvals);

        		/* Get information from the dialog */

        		if (!mandelbrot_dialog())
        			return;
        	...
        	case RUN_NONINTERACTIVE:
        		...
        		if (status == STATUS_SUCCESS)
        			...
        			wvals.colormode = param[8].data.d_int8;
        			wvals.redstretch = param[9].data.d_float;
        			wvals.greenstretch = param[10].data.d_float;
        			wvals.bluestretch = param[11].data.d_float;
        			wvals.redmode = param[12].data.d_int8;
        			wvals.greenmode = param[13].data.d_int8;
        			wvals.bluemode = param[14].data.d_int8;
  ...
  if (status == STATUS_SUCCESS)
    {
      ...
      if (gimp_drawable_color (drawable->id))
        {
          gimp_progress_init ("Rendering mandelbrot fractal...");
...
/*          gimp_tile_cache_ntiles (2 * (drawable->width / gimp_tile_width ()
	  ...
	  if (wvals.colormode==1)  ExternalParam = gimp_run_procedure("plug_in_gradmap",&ExternalInt,
        	...
        	if (run_mode == RUN_INTERACTIVE)
        		gimp_set_data("plug_in_mandelbrot", &wvals, sizeof(mandelbrot_vals_t));
        }
      else
        {
...
static void
mandelbrot_get_pixel(int x, int y, guchar *pixel)
{
        static gint row  = -1;
        static gint col  = -1;
...
} /* mandelbrot_get_pixel */



static void
mandelbrot_render_row (const guchar *src_row,
        	  guchar *dest_row,
        	  gint row,
        	  gint row_width,
        	  gint bytes)
...
static void
mandelbrot_logo_ok_callback(GtkWidget *widget, gpointer   data)
{
...
static void
mandelbrot_about_callback(GtkWidget *widget, gpointer   data)
{
...
build_preview_source_image(void)
{
        ...
        for (y = 0; y < preview_height; y++) {
        	...
        	for (x = 0; x < preview_width; x++) {
        		mandelbrot_get_pixel((int) px, (int) py, pixel);
...
mandelbrot_dialog(void)
{
        ...
        gtk_window_set_title(GTK_WINDOW(dialog), "Mandelbrot Fractal Explorer (cotting@mygale.org)");
               ...
               toggle_vbox = gtk_vbox_new (FALSE, 0);
                  ...
                  gtk_container_add (GTK_CONTAINER (frame), toggle_vbox);
             	  ...
             	  table = gtk_table_new(5, 5, FALSE);
                     ...
                     dialog_create_value("XMIN", GTK_TABLE(table), 0, &wvals.xmin,-10,10);
                     dialog_create_value("XMAX", GTK_TABLE(table), 1, &wvals.xmax,-10,10);
                     dialog_create_value("YMIN", GTK_TABLE(table), 2, &wvals.ymin,-10,10);
                     dialog_create_value("YMAX", GTK_TABLE(table), 3, &wvals.ymax,-10,10);
	             ...
	             frame = gtk_frame_new ("Color density:");
	                ...
	                toggle_vbox = gtk_vbox_new (FALSE, 0);
	                   ...
	                   table6 = gtk_table_new(3, 3, FALSE);
                              ...
                              dialog_create_value("Red", GTK_TABLE(table6), 0, &wvals.redstretch,0,128);
                              dialog_create_value("Green", GTK_TABLE(table6), 1, &wvals.greenstretch,0,128);
                              dialog_create_value("Blue", GTK_TABLE(table6), 2, &wvals.bluestretch,0,128);
                        gtk_widget_show (toggle_vbox);
                     gtk_widget_show (frame);
    ...
    gtk_signal_connect (GTK_OBJECT (toggle), "toggled",
        		(GtkSignalFunc) mandelbrot_toggle_update,
        		&do_redsinus);
...
gtk_container_border_width(GTK_CONTAINER(GTK_DIALOG(dialog)->action_area), 5);
	...
	button = gtk_button_new_with_label("About...");
        GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
        gtk_signal_connect(GTK_OBJECT(button), "clicked",
                     (GtkSignalFunc)mandelbrot_about_callback,button);
...
static void
mandelbrot_toggle_update (GtkWidget *widget,
        		gpointer   data)
{
  int *toggle_val;
...
mandelbrot_logo_dialog()
{
  ...
  gtk_signal_connect(GTK_OBJECT(xbutton), "clicked",
                     (GtkSignalFunc)mandelbrot_logo_ok_callback,
		     xdlg);
