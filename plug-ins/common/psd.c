 ...
 * Revision history:
 *
 *  2004.01.06 / v2.0.9 / Adam D. Moss
 *       Incorporate some of Andy Wallace's fixes for reading PSD files
 *       generated by PanoTools.  I've included the fixes which stop the
 *       plugin actually crashing on PanoTools files, but I've disabled some
 *       of the other fixes (see the BAD_AFW_FIXES #define) for PanoTools
 *       transparency correctness that were causing other vanilla Photoshop
 *       files to regress as garbled.
 *
...
/* set to TRUE if you want Andy Wallis' fixes for reading files from
   PanoTools, FALSE otherwise.  There is probably a better way of fixing
   the problem. Note that these fixes are not tested against files from
   Photoshop apart from a few Photoshop 4.0 LE files and may cause problems
   with files from more recent versions of Photoshop. */
#define AFW_FIXES TRUE
/* These are some of the above fixes from Andy Wallace that make
   PanoTools files load a little better but garble some other perfectly
   ordinary Photoshop files, so they're off by default. --adam */
#define BAD_AFW_FIXES FALSE
...
static
guchar* RGB_to_RGBA (guchar* rgb_data, gint numpix)
{
  guchar* rtn;
  int i,j;
...
#if BAD_AFW_FIXES
		    /* Convert the RGB data for the layer (not the mask) to RGBA */
		    merged_data = RGB_to_RGBA(merged_data,
					      psd_image.layer[lnum].width *
					      psd_image.layer[lnum].height);
		    /* Add alpha - otherwise cannot add layer mask */
		    if(!gimp_layer_add_alpha(layer_ID))
		      IFDBG {printf("\nCannot add alpha channel to layer!\n");fflush(stdout);}
		    /* Add layer mask */
