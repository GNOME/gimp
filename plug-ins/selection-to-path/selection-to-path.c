...
#define PLUG_IN_ROLE   "ligma-selection-to-path"
...
static GList          * sel2path_query_procedures (LigmaPlugIn           *plug_in);
static LigmaProcedure  * sel2path_create_procedure (LigmaPlugIn           *plug_in,
                                                   const gchar          *name);

static LigmaValueArray * sel2path_run              (LigmaProcedure        *procedure,
...
static gboolean         sel2path                  (LigmaImage            *image);


G_DEFINE_TYPE (Sel2path, sel2path, LIGMA_TYPE_PLUG_IN)

LIGMA_MAIN (SEL2PATH_TYPE)
DEFINE_STD_SET_I18N
...
static GList *
sel2path_query_procedures (LigmaPlugIn *plug_in)
{
  return g_list_append (NULL, g_strdup (PLUG_IN_PROC));
}

static LigmaProcedure *
sel2path_create_procedure (LigmaPlugIn  *plug_in,
                           const gchar *name)
{
  ...
  if (! strcmp (name, PLUG_IN_PROC))
    {
      ...
      LIGMA_PROC_ARG_DOUBLE (procedure, "align-threshold",
                            "Align threshold",
                            "Align threshold",
                            0.2, 2.0, 0.5,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "corner-always-threshold",
                            "Corner always threshold",
                            "Corner always threshold",
                            30, 180, 60.0,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "corner-surround",
                         "Corner surround",
                         "Corner surround",
                         3, 8, 4,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "corner-threshold",
                            "Corner threshold",
                            "Corner threshold",
                            0, 180, 100.0,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "error-threshold",
                            "Error threshold",
                            "Error threshold",
                            0.2, 10, 0.4,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "filter-alternative-surround",
                         "Filter alternative surround",
                         "Filter alternative surround",
                         1, 10, 1,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "filter-epsilon",
                            "Filter epsilon",
                            "Filter epsilon",
                            5, 40, 10.0,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "filter-iteration-count",
                         "Filter iteration count",
                         "Filter iteration count",
                         4, 70, 4,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "filter-percent",
                            "Filter percent",
                            "Filter percent",
                            0, 1, 0.33,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "filter-secondary-surround",
                         "Filter secondary surround",
                         "Filter secondary surround",
                         3, 10, 3,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "filter-surround",
                         "Filter surround",
                         "Filter surround",
                         2, 10, 2,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_BOOLEAN (procedure, "keep-knees",
                             "Keep knees",
                             "Keep knees",
                             FALSE,
                             G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "line-reversion-threshold",
                            "Line reversion threshold",
                            "Line reversion threshold",
                            0.01, 0.2, 0.01,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "line-threshold",
                            "Line threshold",
                            "Line threshold",
                            0.2, 4, 0.5,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "reparametrize-improvement",
                            "Reparametrize improvement",
                            "Reparametrize improvement",
                            0, 1, 0.01,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "reparametrize-threshold",
                            "Reparametrize threshold",
                            "Reparametrize threshold",
                            1, 50, 1.0,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "subdivide-search",
                            "Subdivide search",
                            "Subdivide search",
                            0.05, 1, 0.1,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "subdivide-surround",
                         "Subdivide surround",
                         "Subdivide surround",
                         2, 10, 4,
                         G_PARAM_READWRITE);

      LIGMA_PROC_ARG_DOUBLE (procedure, "subdivide-threshold",
                            "Subdivide threshold",
                            "Subdivide threshold",
                            0.01, 1, 0.03,
                            G_PARAM_READWRITE);

      LIGMA_PROC_ARG_INT (procedure, "tangent-surround",
                         "Tangent surround",
                         "Tangent surround",
                         2, 10, 3,
                         G_PARAM_READWRITE);
...
sel2path_run (LigmaProcedure        *procedure,
              LigmaRunMode           run_mode,
              LigmaImage            *image,
              gint                  n_drawables,
              LigmaDrawable        **drawables,
              ...
              gpointer              run_data)
{
  ...
  switch (run_mode)
    ...
    case LIGMA_RUN_INTERACTIVE:
      if (ligma_get_data_size (PLUG_IN_PROC) > 0)
        ligma_get_data (PLUG_IN_PROC, &selVals);
    ...
    case LIGMA_RUN_NONINTERACTIVE:
      selVals.align_threshold             = LIGMA_VALUES_GET_DOUBLE  (args, 0);
      selVals.corner_always_threshold     = LIGMA_VALUES_GET_DOUBLE  (args, 1);
      selVals.corner_surround             = LIGMA_VALUES_GET_INT     (args, 2);
      selVals.corner_threshold            = LIGMA_VALUES_GET_DOUBLE  (args, 3);
      selVals.error_threshold             = LIGMA_VALUES_GET_DOUBLE  (args, 4);
      selVals.filter_alternative_surround = LIGMA_VALUES_GET_INT     (args, 5);
      selVals.filter_epsilon              = LIGMA_VALUES_GET_DOUBLE  (args, 6);
      selVals.filter_iteration_count      = LIGMA_VALUES_GET_INT     (args, 7);
      selVals.filter_percent              = LIGMA_VALUES_GET_DOUBLE  (args, 8);
      selVals.filter_secondary_surround   = LIGMA_VALUES_GET_INT     (args, 9);
      selVals.filter_surround             = LIGMA_VALUES_GET_INT     (args, 10);
      selVals.keep_knees                  = LIGMA_VALUES_GET_BOOLEAN (args, 11);
      selVals.line_reversion_threshold    = LIGMA_VALUES_GET_DOUBLE  (args, 12);
      selVals.line_threshold              = LIGMA_VALUES_GET_DOUBLE  (args, 13);
      selVals.reparameterize_improvement  = LIGMA_VALUES_GET_DOUBLE  (args, 14);
      selVals.reparameterize_threshold    = LIGMA_VALUES_GET_DOUBLE  (args, 15);
      selVals.subdivide_search            = LIGMA_VALUES_GET_DOUBLE  (args, 16);
      selVals.subdivide_surround          = LIGMA_VALUES_GET_INT     (args, 17);
      selVals.subdivide_threshold         = LIGMA_VALUES_GET_DOUBLE  (args, 18);
      selVals.tangent_surround            = LIGMA_VALUES_GET_INT     (args, 19);
  ...
  if (run_mode == LIGMA_RUN_INTERACTIVE)
    ligma_set_data (PLUG_IN_PROC, &selVals, sizeof(SELVALS));
...
sel2path_dialog (SELVALS *sels)
{
  ...
  dlg = ligma_dialog_new (_("Selection to Path Advanced Settings"),
                         PLUG_IN_ROLE,
                         NULL, 0,
           ...
           LigmaImage              *image)
{
  ...
  vectors = ligma_vectors_new (image, _("Selection"));
  ...
  for (j = 0; j < SPLINE_LIST_ARRAY_LENGTH (in_splines); j++)
    {
      ...
      stroke = ligma_vectors_bezier_stroke_new_moveto (vectors,
                                                      END_POINT (seg).x,
                                                      END_POINT (seg).y);
      ...
      for (i = SPLINE_LIST_LENGTH (spline_list); i > 0; i--)
        {
          ...
          if (SPLINE_DEGREE (seg) == LINEAR)
            ligma_vectors_bezier_stroke_lineto (vectors, stroke,
                                               START_POINT (seg).x,
                                               START_POINT (seg).y);
          else if (SPLINE_DEGREE (seg) == CUBIC)
            ligma_vectors_bezier_stroke_cubicto (vectors, stroke,
                                                CONTROL2 (seg).x,
                                                CONTROL2 (seg).y,
                                                CONTROL1 (seg).x,
                                                CONTROL1 (seg).y,
      ...
      ligma_vectors_stroke_close (vectors, stroke);

      /* transform to LIGMAs coordinate system, taking the selections
       * bounding box into account  */
      ligma_vectors_stroke_scale (vectors, stroke, 1.0, -1.0);
      ligma_vectors_stroke_translate (vectors, stroke,
...
sel2path (LigmaImage *image)
{
  ...
  ligma_selection_bounds (image, &has_sel,
  ...
  sel_buffer  = ligma_drawable_get_buffer (LIGMA_DRAWABLE (selection));
