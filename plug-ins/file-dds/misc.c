...
void
decode_ycocg_image (LigmaDrawable *drawable,
                    gboolean      shadow)
{
  ...
  if (shadow)
    {
      sbuffer = ligma_drawable_get_shadow_buffer (drawable);
  ...
  ligma_progress_init ("Decoding YCoCg pixels...");
  ...
  for (i = 0; i < num_pixels; ++i)
    {
      ...
      if ((i & 0x7fff) == 0)
        ligma_progress_update ((float)i / (float)num_pixels);
    }
  ...
  ligma_drawable_update (drawable, 0, 0, w, h);
...
void
decode_ycocg_scaled_image (LigmaDrawable *drawable,
                           gboolean      shadow)
{
  GeglBuffer *buffer, *sbuffer;
  const Babl *format;
  ...
  ligma_progress_init ("Decoding YCoCg (scaled) pixels...");
...
void
decode_alpha_exp_image (LigmaDrawable *drawable,
                        gboolean      shadow)
{
  GeglBuffer *buffer, *sbuffer;
  const Babl *format;
  ...
  ligma_progress_init ("Decoding Alpha-exponent pixels...");
