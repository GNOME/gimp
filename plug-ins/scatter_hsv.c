 ...
 * Time-stamp: <1997/06/08 22:34:32 narazaki@InetQ.or.jp>
 * Version: 0.37
 *
...
#define PROGRESS_NAME	"scatter_hsv (0.37): scattering..."
#define MENU_POSITION	"<Image>/Filters/Image/Scatter HSV"
				/* or "<Image>/Image/Map/Scatter HSV" */
...
DIALOG ()
{
  ...
  frame = gtkW_frame_new (vbox, "Preview (4:1)");
...
scatter_hsv_preview_update ()
{
  int	scale;
  int	x, y, dx, dy;
  guchar data[3], fore[3];
    
  gimp_palette_get_foreground (fore+0, fore+1, fore+2);

  scale = 4;
  for (y = 0; y < PREVIEW_HEIGHT; y += scale)
    for (x = 0; x < PREVIEW_WIDTH/2; x += scale)
      {
	data[0] = fore[0];
	data[1] = fore[1];
	data[2] = fore[2];
	...
	for (dy = 0; dy < scale; dy++)
	  for (dx = 0; dx < scale; dx++)
	    gtk_preview_draw_row (GTK_PREVIEW (preview), data, x+dx, y+dy, 1);
      }
  for (y = 0; y < PREVIEW_HEIGHT; y ++)
    for (x = PREVIEW_WIDTH/2; x < PREVIEW_WIDTH; x++)
      {
	data[0] = fore[0];
	data[1] = fore[1];
	data[2] = fore[2];
