...
static void      warp_one         (GDrawable *src_draw, 
				   GDrawable *dest_draw, 
				   GDrawable *map_x, 
				   GDrawable *map_y  );
...
static gint      displace_map_constrain    (gint32     image_id,
					    gint32     drawable_id,
					    gpointer   data);
static void      displace_map_callback     (gint32     id,
					    gpointer   data);
...
query ()
{
  static GParamDef args[] =
  {
    ...
    { PARAM_FLOAT, "amount", "Displace multiplier" },
    { PARAM_FLOAT, "angle", "Angle of displacement" },
    { PARAM_FLOAT, "iter", "Iteration count" },
    { PARAM_DRAWABLE, "displace_map", "Displacement map" },
  ...
  gimp_install_procedure ("plug_in_warp",
			  "Twist or smear the contents of an image",
  ...
  *return_vals = values;
  ...
  switch (run_mode)
      ...
      if (status == STATUS_SUCCESS)
	{
	  ...
	  dvals.displace_map = param[6].data.d_int32;
  ...
  if (status == STATUS_SUCCESS)
    {
      ...
      gimp_tile_cache_ntiles (TILE_CACHE_SIZE);
         ...
         /*  run the warp effect  */
         displace (drawable, &map_x, &map_y, &xyimage_id, &output_image_id);
  ...
  gimp_image_delete(output_image_id); 
...
displace_dialog (GDrawable *drawable)
{
  ...
  /*  The main table  */
  frame = gtk_frame_new ("Warp Options");
  ...
  label = gtk_label_new ("Vector Rot. Angle");
  gtk_table_attach (GTK_TABLE (table), label, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 0, 0);
  gtk_widget_show (label);

  label = gtk_label_new ("Iterarations");
  ...
  menu = gimp_drawable_menu_new (displace_map_constrain, displace_map_callback,
				 drawable, dvals.displace_map);
...
static void      
displace        (GDrawable *orig_draw,
		  GDrawable **map_x_p,
		  GDrawable **map_y_p,
		  gint32 *xyimage_id_p,
		  gint32 *output_image_id_p    )
{
  GDrawable *disp_map;    /* displacement map, ie, control array */

  gint out_layer_id;  /* Image, Layer IDs for new output image */
  GDrawable *output_d;
  ...
  gint32 ydlayer = -1;
  gint32 xyimage_id = -1;
  gint32 output_image_id = -1;

  gint warp_iter;           /* index var. over all "warp" displacement iterations */

  disp_map = gimp_drawable_get(dvals.displace_map);
  ...
  /* Get selection area */
   ...
   /* -- Create a new image for the output data  -- */

   output_image_id = gimp_image_new(width, height, RGB);

   out_layer_id = gimp_layer_new(output_image_id, "Warp_Output",
			     width, height,
			     RGB_IMAGE, 100.0, NORMAL_MODE);
   gimp_image_add_layer (output_image_id, out_layer_id, 0);

   map_x = gimp_drawable_get(xdlayer);
   map_y = gimp_drawable_get(ydlayer);
   output_d = gimp_drawable_get(out_layer_id);
   /*   orig_draw = gimp_drawable_get(orig_id); */

   /*   warp_one(orig_draw, output_d, map_x, map_y); */
   /*   gimp_display_new(output_image_id); */
   /*   warp_one(output_d, orig_draw, map_x, map_y); */
  ...
  *output_image_id_p = output_image_id;
  *map_x_p = map_x;
  *map_y_p = map_y;
	 ...
	 GDrawable *map_y  )
{
  ...
  /* Get selection area */
   ...
   gimp_drawable_mask_bounds (src_draw->id, &x1, &y1, &x2, &y2);
   width  = src_draw->width;
   height = src_draw->height;
   dest_bytes  = src_draw->bpp;

   /*   fprintf(stderr,"Source bounds = %d %d %d %d\n",x1,y1,x2,y2); */
   ...
   /*   fprintf(stderr,"calling rgn_init\n");  */
 
   gimp_pixel_rgn_init (&src_rgn, src_draw, x1, y1, (x2 - x1), (y2 - y1), FALSE, FALSE);
   gimp_pixel_rgn_init (&dest_rgn, dest_draw, x1, y1, (x2 - x1), (y2 - y1), TRUE, TRUE);

  /*  Register the pixel region  */

   pr = gimp_pixel_rgns_register (4, &src_rgn, &dest_rgn, &map_x_rgn, &map_y_rgn);

   /*   fprintf(stderr,"called register\n"); */

   max_progress = (x2 - x1) * (y2 - y1) * 2;
  
   for (pr = pr; pr != NULL; pr = gimp_pixel_rgns_process (pr))
    {
      /*      fprintf(stderr,"top of PR loop.\n"); */
      ...
      for (y = dest_rgn.y; y < (dest_rgn.y + dest_rgn.h); y++)
	{
	  ...
	  for (x = dest_rgn.x; x < (dest_rgn.x + dest_rgn.w); x++)
	    {
	      dx = (displace_map_give_value(mx, xm_alpha, xm_bytes)-127.5) / 127.5; 
	      dy = (displace_map_give_value(my, ym_alpha, ym_bytes)-127.5) / 127.5;
	      ...
	      tile = displace_pixel (src_draw, tile, width, height, x1, y1, x2, y2, xi, yi, &row, &col, pixel[0]);
	      tile = displace_pixel (src_draw, tile, width, height, x1, y1, x2, y2, xi + 1, yi, &row, &col, pixel[1]);
	      tile = displace_pixel (src_draw, tile, width, height, x1, y1, x2, y2, xi, yi + 1, &row, &col, pixel[2]);
	      tile = displace_pixel (src_draw, tile, width, height, x1, y1, x2, y2, xi + 1, yi + 1, &row, &col, pixel[3]);
   ...
   /*  update the region  */
   gimp_drawable_flush (dest_draw);
   gimp_drawable_merge_shadow(dest_draw->id, TRUE);
   gimp_drawable_update (dest_draw->id, x1, y1, (x2 - x1), (y2 - y1));

   /* if (run_mode != RUN_NONINTERACTIVE) */
     gimp_displays_flush();

    /*  detach from the displacement map drawables  */
} /* warp_one */
...
static void
displace_map_callback (gint32     id,
			 gpointer   data)
{
  dvals.displace_map = id;
}
