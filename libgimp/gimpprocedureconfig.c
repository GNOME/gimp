 ...
 * persistency of the last used arguments across LIGMA sessions.
 *
...
struct _LigmaProcedureConfigPrivate
{
...
static void   ligma_procedure_config_constructed   (GObject      *object);
static void   ligma_procedure_config_dispose       (GObject      *object);
static void   ligma_procedure_config_set_property  (GObject      *object,
                                                   guint         property_id,
                                                   const GValue *value,
                                                   GParamSpec   *pspec);
static void   ligma_procedure_config_get_property  (GObject      *object,
                                                   guint         property_id,
                                                   GValue       *value,
                                                   GParamSpec   *pspec);


G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE (LigmaProcedureConfig, ligma_procedure_config,
                                     G_TYPE_OBJECT)

#define parent_class ligma_procedure_config_parent_class
...
{
  const gchar           *name;
  LigmaMetadataSaveFlags  flag;
}
metadata_properties[] =
{
  { "save-exif",          LIGMA_METADATA_SAVE_EXIF          },
  { "save-xmp",           LIGMA_METADATA_SAVE_XMP           },
  { "save-iptc",          LIGMA_METADATA_SAVE_IPTC          },
  { "save-thumbnail",     LIGMA_METADATA_SAVE_THUMBNAIL     },
  { "save-color-profile", LIGMA_METADATA_SAVE_COLOR_PROFILE },
  { "save-comment",       LIGMA_METADATA_SAVE_COMMENT       }
};


static void
ligma_procedure_config_class_init (LigmaProcedureConfigClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed  = ligma_procedure_config_constructed;
  object_class->dispose      = ligma_procedure_config_dispose;
  object_class->set_property = ligma_procedure_config_set_property;
  object_class->get_property = ligma_procedure_config_get_property;
...
static void
ligma_procedure_config_init (LigmaProcedureConfig *config)
{
  config->priv = ligma_procedure_config_get_instance_private (config);

  config->priv->run_mode = -1;
}

static void
ligma_procedure_config_constructed (GObject *object)
{
  LigmaProcedureConfig *config = LIGMA_PROCEDURE_CONFIG (object);

  G_OBJECT_CLASS (parent_class)->constructed (object);

  g_assert (LIGMA_IS_PROCEDURE (config->priv->procedure));
}

static void
ligma_procedure_config_dispose (GObject *object)
{
  LigmaProcedureConfig *config = LIGMA_PROCEDURE_CONFIG (object);
...
static void
ligma_procedure_config_set_property (GObject      *object,
                                    guint         property_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
...
static void
ligma_procedure_config_get_property (GObject    *object,
                                    guint       property_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
...
/**
 * ligma_procedure_config_get_procedure:
...
/**
 * ligma_procedure_config_set_values:
 * @config: a #LigmaProcedureConfig
 * @values: a #LigmaValueArray
 *
 ...
 **/
void
ligma_procedure_config_set_values (LigmaProcedureConfig  *config,
                                  const LigmaValueArray *values)
{
  ...
  g_return_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config));
  g_return_if_fail (values != NULL);

  pspecs = g_object_class_list_properties (G_OBJECT_GET_CLASS (config),
                                           &n_pspecs);
  ligma_procedure_get_aux_arguments (config->priv->procedure, &n_aux_args);
  n_values = ligma_value_array_length (values);
  ...
  for (i = 0; i < n_values; i++)
    {
      ...
      GValue     *value = ligma_value_array_index (values, i);
...
/**
 * ligma_procedure_config_get_values:
 * @config: a #LigmaProcedureConfig
 * @values: a #LigmaValueArray
 *
 ...
 **/
void
ligma_procedure_config_get_values (LigmaProcedureConfig  *config,
...
static void
ligma_procedure_config_get_parasite (LigmaProcedureConfig *config,
                                    GParamSpec          *pspec)
{
  ...
  parasite = ligma_image_get_parasite (config->priv->image, pspec->name);

  if (parasite)
    {
      guint32 parasite_size;

      value = (gchar *) ligma_parasite_get_data (parasite, &parasite_size);
  ...
  if (! value)
    {
      /*  special case "ligma-comment" here, yes this is bad hack  */
      if (! strcmp (pspec->name, "ligma-comment"))
        {
          value = ligma_get_default_comment ();
        }
    }
...
static void
ligma_procedure_config_set_parasite (LigmaProcedureConfig *config,
                                    GParamSpec          *pspec)
{
  LigmaParasite *parasite;
  gchar        *value;
  ...
  if (parasite)
    {
      ...
      image_value = (gchar *) ligma_parasite_get_data (parasite, &parasite_size);
      ...
      if (g_strcmp0 (value, image_value))
        {
          if (value && strlen (value))
            {
              parasite = ligma_parasite_new (pspec->name,
                                            LIGMA_PARASITE_PERSISTENT,
                                            strlen (value) + 1,
                                            value);
              ligma_image_attach_parasite (config->priv->image,
                                          parasite);
              ligma_parasite_free (parasite);
            }
          else
            {
              ligma_image_detach_parasite (config->priv->image,
                                          pspec->name);
            }
        }
  ...
  else
    {
      ...
      if (! strcmp (pspec->name, "ligma-comment"))
        {
          default_value = ligma_get_default_comment ();
        }
...
/**
 * ligma_procedure_config_begin_run:
 * @config:   a #LigmaProcedureConfig
 * @image: (nullable): a #LigmaImage or %NULL
 * @run_mode: the #LigmaRunMode passed to a [class@Procedure]'s run()
 * @args:     the #LigmaValueArray passed to a [class@Procedure]'s run()
 *
 * Populates @config with values for a [class@Procedure]'s run(),
 * depending on @run_mode.
 *
 * If @run_mode is %LIGMA_RUN_INTERACTIVE or %LIGMA_RUN_WITH_LAST_VALS,
 ...
 *
 * If @run_mode is %LIGMA_RUN_NONINTERACTIVE, the contents of @args are
 ...
 * %LIGMA_ARGUMENT_SYNC_PARASITE with [class@Procedure.set_argument_sync]:
 *
 * String properties are set to the value of the image parasite if
 * @run_mode is %LIGMA_RUN_INTERACTIVE or %LIGMA_RUN_WITH_LAST_VALS, or
 * if the corresponding argument is an auxiliary argument. As a
 * special case, a property named "ligma-comment" will default to
 * [func@get_default_comment] if there is no "ligma-comment" parasite.
 *
 ...
 **/
void
ligma_procedure_config_begin_run (LigmaProcedureConfig  *config,
                                 LigmaImage            *image,
                                 LigmaRunMode           run_mode,
                                 const LigmaValueArray *args)
{
  ...
  switch (run_mode)
    ...
    case LIGMA_RUN_WITH_LAST_VALS:
      if (image)
        {
          loaded = ligma_procedure_config_load_parasite (config, image,
                                                        &error);
          if (! loaded && error)
            {
      ...
      if (! loaded &&
          ! ligma_procedure_config_load_last (config, &error) && error)
        {
    ...
    case LIGMA_RUN_NONINTERACTIVE:
      ligma_procedure_config_set_values (config, args);
      break;
    }
  ...
  for (i = 0; i < n_pspecs; i++)
    {
      ...
      switch (ligma_procedure_get_argument_sync (config->priv->procedure,
                                                pspec->name))
        {
        case LIGMA_ARGUMENT_SYNC_PARASITE:
           ...
           *  procedure
           */
          if (ligma_procedure_find_aux_argument (config->priv->procedure,
                                                pspec->name) ||
              (run_mode != LIGMA_RUN_NONINTERACTIVE))
            {
              ligma_procedure_config_get_parasite (config, pspec);
            }
          break;
...
/**
 * ligma_procedure_config_end_run:
 * @config: a #LigmaProcedureConfig
 ...
 * %LIGMA_RUN_INTERACTIVE, @config is saved as last used values to be
 * used when the procedure runs again. Additionally, if the #LigmaImage
 ...
 * #LigmaParasite and [method@Image.attach_parasite].
 *
 * If @run_mode was not %LIGMA_RUN_NONINTERACTIVE, this function also
 * conveniently calls [func@displays_flush], which is what most
 * procedures want and doesn't do any harm if called redundantly.
 *
 * After a %LIGMA_RUN_INTERACTIVE run, %LIGMA_ARGUMENT_SYNC_PARASITE
 ...
 **/
void
ligma_procedure_config_end_run (LigmaProcedureConfig *config,
                               LigmaPDBStatusType    status)
{
  g_return_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config));

  if (config->priv->run_mode != LIGMA_RUN_NONINTERACTIVE)
    ligma_displays_flush ();

  if (status == LIGMA_PDB_SUCCESS &&
      config->priv->run_mode == LIGMA_RUN_INTERACTIVE)
    {
      GParamSpec **pspecs;
      guint        n_pspecs;
      gint         i;
      ...
      if (config->priv->image)
        ligma_procedure_config_save_parasite (config, config->priv->image,
                                             NULL);

      if (! ligma_procedure_config_save_last (config, &error))
        {
      ...
      for (i = 0; i < n_pspecs; i++)
        {
          ...
          switch (ligma_procedure_get_argument_sync (config->priv->procedure,
            ...
            {
            case LIGMA_ARGUMENT_SYNC_PARASITE:
              ligma_procedure_config_set_parasite (config, pspec);
              break;
...
/**
 * ligma_procedure_config_begin_export:
 * @config:         a #LigmaProcedureConfig
 ...
 *
 * - "save-exif" for %LIGMA_METADATA_SAVE_EXIF.
 * - "save-xmp" for %LIGMA_METADATA_SAVE_XMP.
 * - "save-iptc" for %LIGMA_METADATA_SAVE_IPTC.
 * - "save-thumbnail" for %LIGMA_METADATA_SAVE_THUMBNAIL.
 * - "save-color-profile" for %LIGMA_METADATA_SAVE_COLOR_PROFILE.
 * - "save-comment" for %LIGMA_METADATA_SAVE_COMMENT.
 *
 ...
 * LIGMA_PROC_ARG_BOOLEAN() or LIGMA_PROC_AUX_ARG_BOOLEAN().
 *
                                    ...
                                    const gchar          *mime_type)
{
  ...
  g_return_val_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config), NULL);
  g_return_val_if_fail (LIGMA_IS_IMAGE (original_image), NULL);
  ...
  if (mime_type)
    {
      ...
      config->priv->metadata =
        ligma_image_metadata_save_prepare (original_image,
                                          mime_type,
                                          &metadata_flags);
  ...
  ligma_procedure_config_begin_run (config, original_image, run_mode, args);

  return config->priv->metadata;
}

/**
 * ligma_procedure_config_end_export:
 * @config:         a #LigmaProcedureConfig
 ...
 * provides additional features to automate file export:
 *
 * If @status is %LIGMA_PDB_SUCCESS, and
 ...
 **/
void
ligma_procedure_config_end_export (LigmaProcedureConfig *config,
                                  LigmaImage           *exported_image,
                                  GFile               *file,
                                  LigmaPDBStatusType    status)
{
  g_return_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config));
  g_return_if_fail (LIGMA_IS_IMAGE (exported_image));
  g_return_if_fail (G_IS_FILE (file));

  if (status == LIGMA_PDB_SUCCESS)
    {
      ligma_procedure_config_save_metadata (config, exported_image, file);
    }
...
/**
 * ligma_procedure_config_save_metadata:
 * @config:         a #LigmaProcedureConfig
 * @exported_image: the image that was actually exported
 ...
 **/
void
ligma_procedure_config_save_metadata (LigmaProcedureConfig *config,
                                     LigmaImage           *exported_image,
                                     GFile               *file)
{
  g_return_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config));
  ...
  if (config->priv->metadata && ! config->priv->metadata_saved)
    {
      ...
      for (i = 0; i < G_N_ELEMENTS (metadata_properties); i++)
        {
          const gchar           *prop_name = metadata_properties[i].name;
          LigmaMetadataSaveFlags  prop_flag = metadata_properties[i].flag;
          GParamSpec            *pspec;
          gboolean               value;
      ...
      if (! ligma_image_metadata_save_finish (exported_image,
...
static GFile *
ligma_procedure_config_get_file (LigmaProcedureConfig *config,
                                const gchar         *extension)
{
  ...
  file = ligma_directory_file ("plug-in-settings", basename, NULL);
  g_free (basename);
...
gboolean
ligma_procedure_config_has_default (LigmaProcedureConfig *config)
{
  GFile    *file;
  gboolean  success;

  g_return_val_if_fail (LIGMA_IS_PROCEDURE_CONFIG (config), FALSE);

  file = ligma_procedure_config_get_file (config, ".default");
...
gboolean
ligma_procedure_config_load_default (LigmaProcedureConfig  *config,
                                    GError              **error)
{
  GFile    *file;
  gboolean  success;
  ...
  success = ligma_config_deserialize_file (LIGMA_CONFIG (config),
                                          file,
                                          NULL, error);

  if (! success && error && (*error)->code == LIGMA_CONFIG_ERROR_OPEN_ENOENT)
    {
      g_clear_error (error);
    }
...
gboolean
ligma_procedure_config_save_default (LigmaProcedureConfig  *config,
                                    GError              **error)
{
  GFile    *file;
  gboolean  success;
  ...
  success = ligma_config_serialize_to_file (LIGMA_CONFIG (config),
                                           file,
                                           "settings",
                                           "end of settings",
                                           NULL, error);
...
gboolean
ligma_procedure_config_load_last (LigmaProcedureConfig  *config,
                                 GError              **error)
{
  GFile    *file;
  gboolean  success;
  ...
  file = ligma_procedure_config_get_file (config, ".last");

  success = ligma_config_deserialize_file (LIGMA_CONFIG (config),
                                          file,
                                          NULL, error);

  if (! success && (*error)->code == LIGMA_CONFIG_ERROR_OPEN_ENOENT)
    {
      g_clear_error (error);
    }
...
gboolean
ligma_procedure_config_save_last (LigmaProcedureConfig  *config,
                                 GError              **error)
{
  GFile    *file;
  gboolean  success;
...
static gchar *
ligma_procedure_config_parasite_name (LigmaProcedureConfig *config,
                                     const gchar         *suffix)
{
...
gboolean
ligma_procedure_config_load_parasite (LigmaProcedureConfig  *config,
                                     LigmaImage            *image,
                                     GError              **error)
{
  gchar        *name;
  ...
  name = ligma_procedure_config_parasite_name (config, "-last");
  parasite = ligma_image_get_parasite (image, name);
  g_free (name);

  if (! parasite)
    return FALSE;

  success = ligma_config_deserialize_parasite (LIGMA_CONFIG (config),
                                              parasite,
                                              NULL, error);
  ligma_parasite_free (parasite);
...
gboolean
ligma_procedure_config_save_parasite (LigmaProcedureConfig  *config,
                                     LigmaImage            *image,
                                     GError              **error)
{
  gchar        *name;
  ...
  name = ligma_procedure_config_parasite_name (config, "-last");
  parasite = ligma_config_serialize_to_parasite (LIGMA_CONFIG (config),
                                                name,
                                                LIGMA_PARASITE_PERSISTENT,
                                                NULL);
  g_free (name);
