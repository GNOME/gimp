...
#define GIMP_CONST_STRING_TEST(testname, test) \
  ...
  /* libgimpbase/gimpparasiteio.h is not listed in gimpbase.h.
   * pixpipe is only handled in app, but this const, a struct and
   * several functions are available through Gobject Introspection so we
   * can't just remove it from the API.
   * The functions have been deprecated and will be made private in the future.
   *
