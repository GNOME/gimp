...
if ! git ls-remote --exit-code upstream >/dev/null 2>&1 ; then
    git remote add upstream https://gitlab.gnome.org/${CI_MERGE_REQUEST_PROJECT_PATH:-$CI_PROJECT_PATH}.git
fi
...
git rev-list --first-parent "upstream/${CI_MERGE_REQUEST_TARGET_BRANCH_NAME:-gimp-3-2}" > "temp_upstream" 2>&1
