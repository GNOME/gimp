            ...
            if (filename_utf16 != NULL)
              {
                ...
                profile_handle = OpenColorProfileW (&win_profile, PROFILE_READ,
                                                    FILE_SHARE_READ, OPEN_EXISTING);
                if (profile_handle)
                  {
                    DWORD profile_size = 0;

                    GetColorProfileFromHandle (profile_handle, NULL, &profile_size);
                    if (profile_size > 0)
                      {
                        guint8 *profile_data = g_malloc (profile_size);

                        if (GetColorProfileFromHandle (profile_handle, profile_data, &profile_size))
                          {
                    ...
                    CloseColorProfile (profile_handle);
                  }
